...
--   amount 必须**逐分等于**各单该收的钱之和（少一分多一分都拒）；
--   「该收多少」在 services/order_money.py 一处算：整单核销 = 这一单的**欠款** arrears
--   （= 应收 − 净已收；收过一部分的单要能收剩下那部分），按商品核销 = 点名那几行的应收。
--   ⚠️ 曾经写的是 receivable（当时卖了多少）：对按商品核销过一部分的单偏大 →
--     客户端按欠款发、后端按应收算，两边对不上（这种单再也收不动）；凑巧对上就是**多收**。
--   ⚠️ 一张收款单可以**带多张单**（账本页「点合计 → 核销全部」就是这么发的）：
--     一张收款单 + 逐单各一条现金流水（报表按流水逐条求和）。
