...
**什么时候动哪一位（2026-09-21 用户亲自定的口径，照这个说、照这个改）**：

| 位 | 什么时候 +1 | 例子 |
| --- | --- | --- |
| 第 1 位 `MAJOR` | **超级大的版本更新**（产品换形态、大改版） | `0.x.y` → `1.0.0` |
| 第 2 位 `MINOR` | **加了很多新功能** | `0.2.0` → `0.3.0`（这一轮的退货申请 / 运费分类 / 司机计费规则就属于这一档） |
| 第 3 位 `PATCH` | **修了一批 bug / 小修小补 / 加个小功能** | `0.2.0` → `0.2.1` |

⚠️ 改产品版本 = **改仓库根 `VERSION` 一个文件**（三端同源：后端 `config.py`、前端 `package.json`、
App 的 `versionName` 都读它）。不要只改 App，也不要另起一个 `1.0.0.<日期>` 式的自编号。
