...
| `a8cdc5d`（本报告重写版那个提交） | ❌ | ✅ | ⚠️ 只有安卓端到端红 —— 这一次是**另一种**：注解原文「**连 `/tmp/e2e.rc` 都没有 —— 模拟器压根没起来**」＝**这次没有跑**（不是流程失败）。同一提交只动了两份 `.md` |
  ...
  ⭐ 当天合计：这个「安卓端到端」作业跑了 **10 次：绿 6 次、`rc=1` 红 4 次、无 rc 红 2 次**。
  · `rc=1` ＝ **脚本真的跑了，流程里某一段失败**（`97c5caa`、`c4c8206`、`cc47565`，以及更早一次）；
  · 无 `rc` ＝ **模拟器压根没起来**（`5cb27fb`、`a8cdc5d`）—— 「没跑」不等于「通过」。
...
**只对表里列出的那个提交**成立。⚠️ 反例就在上表里：`97c5caa`（只改了两处 `.md`）上那个作业红着（**流程里某一段失败**），
