...
| `c4c8206`（演练工具那个提交） | ❌ | ✅ | ⚠️ 只有**「安卓端到端」作业**红：注解原文是「**rc=1** —— 见上面日志与 `_agent/e2e` 里的截图」⇒ 按工作流自己的分档（0 通过 / 3 带理由跳过 / 9 哨兵 / 其它＝脚本跑了但失败）**这是「脚本真的跑了并报失败」**，⛔ 不是「模拟器没起来」（那是「连 `/tmp/e2e.rc` 都没有」那一支）。该提交只动了 `_tools/ops/_drill.py` 与三个 `.md`；静态检查 / 后端用例 / AI 对账 / 安卓单测全绿。⚠️ **失败在哪一段读不到**（日志要登录）—— 已把 `/tmp/e2e.log` 尾部加进注解，下一次红就能读出是哪一段 |

  ⭐ 当天合计：这个「安卓端到端」作业跑了 **8 次：绿 5 次、红 3 次**（`97c5caa`、`c4c8206`、`cc47565`；三次都是 **rc=1**
  = 脚本跑了但某一段失败；三次的提交都只改文档或工具脚本，⛔ 都不碰安卓）。另有一次是「连 rc 都没有」= 模拟器没起
  来（`5cb27fb` 那一次）。⚠️ **两种红的含义完全不同**，台账不再把它们混成一句「基础设施问题」。

⛔ **这一节按提交记，不按「tip」记**：每次推新提交都会产生新的运行 —— 所以「现在的 tip 是绿的」这件事
**只对表里列出的那个提交**成立。⚠️ 反面例子就在上表里：`97c5caa` 上有一个作业红着（红在**基础设施**那一格：
模拟器没起来），而下一次推的 `dadf095a` 同样的工作流就整轮绿了 —— ⛔ 所以既不许拿一次红判定代码有问题，
也不许拿一次绿保证下一次绿。
