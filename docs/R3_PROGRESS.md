...
| `97c5caa` | ❌ | ✅ | ⚠️ **只有「安卓端到端（登录 → 导航 → 下单）」作业红**：模拟器在托管 runner 上没起来（KVM/资源那种老毛病 —— 同一个作业在 `cc949bf`、`684a937` 上是绿的，而这个提交只动了两处 `.md`）。同一提交的静态检查 / 后端用例 / AI 权限对账 / 安卓单测全绿 |
| `dadf095a`（这一节的收尾提交） | ✅ | ✅ | **整轮绿**（含安卓端到端那个作业 —— 也就是上一次红的那一个，这次模拟器起来了。同一份工作流、只差一个 `md` 提交）|
| `a591ff2` / `1c2c572` | ✅ / ✅ | ✅ / ✅ | 两次整轮绿（后一次是「发布工具 + 护栏自检」那个提交）|
| `c4c8206`（演练工具那个提交） | ❌ | ✅ | ⚠️ 又是**只有「安卓端到端」作业红**：该提交只动了 `_tools/ops/_drill.py` 与三个 `.md`，⛔ 不碰安卓、不碰后端 API、不碰工作流；静态检查 / 后端用例 / AI 对账 / 安卓单测全绿。**推断**是模拟器那一格（基础设施）—— ⛔ 说「推断」是因为 Actions 的作业日志要登录才看得到，读不到确切失败原因 |

  ⭐ 当天合计：这个「安卓端到端」作业跑了 7 次，绿 5 次、红 2 次（红的两次：`97c5caa`、`c4c8206`，两次都只改文档或工具脚本）。
