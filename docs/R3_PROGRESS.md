...
| `a896c48`（台账口径修复 + 报告 v2.1） | ✅ | ✅ | **整轮绿**（含安卓端到端）—— 本节最后一行 |
  ...
  ⭐ 当天合计：这个「安卓端到端」作业跑了 **17 次：绿 9 次、`rc=1` 红 5 次、无 rc 红 3 次**
  （⚠️ 这是**截至表里最后一行（`a896c48`）**的数 —— 口径：上表列出的每个提交各算一次，
  `cancelled` 的不计入；⛔ 每推一次就会变，**别手抄，重数**）。
  · `rc=1` ＝ **脚本真的跑了，流程里某一段失败**（`bcb11a9`、`97c5caa`、`c4c8206`、`cc47565`、`5fa3526`）；
