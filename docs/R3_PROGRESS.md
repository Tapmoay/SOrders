...
- ❌ **反向验证必须完整还原**（R3-07b，**只补到一半**）—— 复现：`python _tools/qa/_check_reverse_verify_restore.py`
  ...
  · **L2 跑完**逐字节比对**证明一模一样**：**94/143** —— 下限 94。⛔ **剩下的 49 份才是缺口**：
  ...
  ⛔ **2026-09-26 修了两件事，都要记清楚（不是「缺口改小了」）**：
    ...
    ② 又给 6 份补上「还原**当场核对**」（`geocode` / `card_claim` / `ctx_budget` / `cost_history` /
    `image_refs` / `billing`：还原后立刻 `read_text() != original` 就记账），六份各自跑过一遍，
    全绿（证明没污染源码树）→ L2 88 → **94**，棘轮跟着抬到 94。
  ...
  ⚠️ 剩下的 49 份**不是一轮能补完的**（每份形状不同，要逐份改 + 逐份跑），棘轮会盯着它只增不减。
