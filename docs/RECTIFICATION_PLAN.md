...
### 库内不变式审计：证明它**真的会报缺陷**，并当场抓到一条**死了半边的判据**（2026-09-25 第 48 轮）

**为什么挑它**：报告 §19 的原话是「**Domain + Database invariants 才是真正的业务真相**」，而
`_tools/fuzz/_fuzz_invariants.py` 是本项目唯一「直接查库、把两张表摆一起看对不对得上」的工具。
它自己的文件头就记着这类事故：**5 条订单状态判据写成小写 → 永远扫描 0 行、永远报绿**，
改对大小写后立刻命中真缺陷。也就是说：**它不报缺陷有两种可能** —— ① 真的没问题；② 那条判据死了。
工具自己分不开这两者，所以由**外部**来分。

**做法**（全程不碰本机开发库）：新增 `_tools/fuzz/_reverse_verify_invariants.py` ——
把本机库**复制**到临时目录，每条破坏**重新复制一份**、只改坏一处，再用 `SORDERS_DB=副本` 跑审计，
断言输出里出现**那一条**判据的缺陷行。5 条 → **6/6 全成立**：
① 已送达的单没了送达时间 ② 派单中/已接单的单没有司机 ③ 商品行金额出现负数 ④ 账本流水指向不存在的订单
⑤ **工具自己**的状态字面量写错 → 自检当场拦下。

**第 ② 条当场没报 → 抓到一条死了半边的判据**：
`where upper(status) in ('DISPATCHED', 'ACKED')` —— **`ACKED` 在 `OrderStatus` 里根本不存在**
（接单是 `ACCEPTED`）。于是「派单中/已接单但没有司机」**只对 DISPATCHED 有效**，ACCEPTED 那半边
永远为假，而它看起来像覆盖了两种状态。这正是这个文件自己抱怨过的手写清单腐烂（它上面那段注释
刚说完 `RECALLED` 的事）—— **同一个文件、同一种病、长在另一个判据上**。

**结构性修法**（不是只改那个字面量）：给这个工具加一条**自检** ——
本文件 SQL 里出现的每个状态字面量，都必须能在 `enums.py` 的 `OrderStatus` 里找到，
或者写进一张**带理由的放行表**（inventory_movements / driver_settlements / driver_bills 那几张表的状态）；
放行表还必须**条条命中**（防化石）；认出的字面量少于 8 个也报错（解析器失配）。

⛔ 自检扫的是 **AST 里「真的会执行的字符串」**，不含文档字符串 —— 因为本文件的文档里**故意**引用了
旧的坏字面量（第 125 行那句 `('CANCELLED','DELIVERED','RECALLED')` 举的正是「原来写错了」的例子）。
这是「判据被自己的文档搞红/搞绿」的**第 4 次**，只不过这次提前躲开了 ——
写第 ⑤ 条注入时又踩了一次：`'CANCELLED'` 在文件里第一次出现是在**文档**里，替换到那儿等于没改。

**修完之后跑一遍本机库**：42 项检查 / **确认缺陷 0** —— 库里没有「ACCEPTED 且没有司机」的行，
所以这条判据是**从"永远为假"变成"真的在查、而且现在查出来是干净的"**。

### 导出产物那条安全红线：反向验证一口气抓到**两个洞**（2026-09-25 第 49 轮）

`_check_export_guards.py` 守的是**匿名可下载**：`/static/uploads/` 在生产是 nginx 用 alias
**直出磁盘**的（请求根本不到 uvicorn），所以应用层那句 404 拦不住它 —— 历史产物（老文件名可枚举）
匿名就能拖走别人的账本。这条红线此前**一条反向验证都没有**。

新增 `_tools/qa/_reverse_verify_export_guards.py`（5 条注入 → **6/6**）：
① 后端又出现 `makedirs("…/uploads/exports")`；② 部署脚本里那条 deny 没了；
③ 保留治理里**函数还在、任务表那行没了**；④ 定位函数被改名；⑤ 扫描器失配（判据空转）。

**它当场抓到两个洞，而且都在「字符串 in 源码」这种判据上**：

| 洞 | 原来为什么是绿的 | 现在的判据 |
| --- | --- | --- |
| **③ 死函数** | 只查 `"purge_old_export_files" in 源码` —— **一个定义就够了**。把任务表那一行删掉（函数在、没人调它）判据照样绿，而后果是导出产物**永远不会被清理** | 要求 **`def purge_old_export_files(`（实现）** + **`, purge_old_export_files)`（真的挂进每日任务表）**，并且判在 **`code_only`** 上（注释里写着那行不算 —— 第一版就是这么被我自己的注入骗绿的） |
| **② 半截 deny** | 只查 `"/static/uploads/exports/" in 源码` —— 而部署脚本里这段片段出现**四处**（文档字符串的举例、nginx 片段、`re.subn` 的替换串、print），删掉真正要写进 conf 的那一处，另外三处照样满足它 | 只看**真的会执行的字符串**（AST 排除文档字符串），而且要求 deny 片段出现**两次**：这个脚本有两条路（新机器的 `NEW_BLOCK` + 老机器的 `re.subn` 插入），少一条 = 走那条路的机器仍然匿名可下载 |

> ⚠️ 这是「判据被自己的文档/注释满足」的**第 5、6 次**（前四次：`checkfirst`、`emit_notification`、
> `_mysql_session_utc`、一段 SQL 的文档举例）。六次里有五次长在**同一类判据**上：
> **`"某个字符串" in 源码`**。可操作的规矩已经写进上面几节：判据锚点要用**代码形状**
> （函数定义 / 调用形状 / 赋值），拿不准就 `code_only`；判「某段文本在不在」时必须排除文档字符串。

### 审计卡片那条红线补反向验证（第 ③ 条又抓到注释洞）+ 让工具的 stderr 也按 UTF-8 写（2026-09-25 第 50 轮）

**A. `_tools/ai/_check_action_labels.py`**（守的是用户报过的缺陷：审计卡片上出现
`USER_RESTORE` / `PRODUCT_DELETE` 这种原始码，「用户根本就看不懂」）。新增
`_reverse_verify_action_labels.py`（5 条注入 → **6/6**）：

| # | 注入 | 结果 |
| --- | --- | --- |
| ① | 后端新加一个动作、App 那张表没跟上 | ✅ 红（这条就是缺陷本身） |
| ② | App 侧删掉一条中文 | ✅ 红 |
| ③ | **把那条中文注释掉** | ⛔ **修前是绿的** → 已修，现在红 |
| ④ | `actionLabel` 被改名/搬走 | ✅ 红（判据硬失败，不静默放行） |
| ⑤ | 解析器失配（后端一个动作都扫不到） | ✅ 红（反空转下限） |

第 ③ 条是真洞：把 `// "ORDER_CREATE" -> "新建订单"` 注释掉，正则**照样在注释里命中** ——
后果正是这条红线要防的那件事（卡片上又显示原始码）。修法：新增 `_strip_kotlin_comments()`
（**保留字符串里的 `//`**），匹配前先剥注释。这是同一类洞的第 **7** 次。

**B. 顺带发现并修掉一个系统性小毛病：stderr 没按 UTF-8 写**

第 ④ 条一开始是**假 MISS**：`raise SystemExit("❌ 找不到 actionLabel…")` 走的是 **stderr**，
而这个脚本只重配了 stdout → Windows 上按 **GBK** 写 → 反向验证断言里的中文匹配不上
（人看到的也是乱码）。扫了一遍 `_tools/**`：**80 个脚本只重配了 stdout、10 个两个都没重配**。

⛔ **不逐文件改**（90 个文件的机械 diff 不划算）：改 **4 个共用模块**
（`_airepo` / `_check_hints` / `_check_single_source` / `_fuzzlib`）—— 绝大多数工具都 import 它们，
**导入即生效**。
⚠️ 第一版补丁把片段插在 `from pathlib import Path` 之后，而 `_airepo.py` **根本没有 `import sys`**
→ 所有 import 它的工具当场全炸（`_check_all.py` 跑出**空输出**）。补上 `import sys` 后复跑 **99/99**。
验证：`import _airepo` 之后往 stderr 写中文 → **可读**（改前是乱码）。

### 「截断接线」那条红线正在**悄悄失效**，本轮抓住了（2026-09-25 第 51 轮）

`_check_page_truncation_wiring.py` 守的是代价最大的一类静默：**列表被服务端截断而界面不说** ——
用户照着一个错的结论做决定（现金流水页把"一页 N 条"当总额求和，实测 **少算 62%**；账号列表超过一页 →
「没有这个账号」→ 再建一个、撞唯一约束）。

它的"7 个页面都还接着"是用**数量下限**表达的，注释写着「下限＝本轮的实测值 …**留余量等于允许某个
页面静默退回去**」。可是页面一直在加、下限没同步：**下限停在 8 / 9，实测已经 14 / 16** ——
也就是说**中间那 6 处可以静默退回去，而检查照样绿**。这条红线正在慢慢变成"看着像在查"。

**修法（不留余量 + 逼同步）**：下限改成**当前实测值**（14 / 16），并新增一条**下限过期就报红**的判据 ——
实测 > 下限时提示「把 MIN_* 改成实测值」。于是增长时必须同步这一行，余量永远不存在。

**新增反向验证 `_reverse_verify_page_truncation_wiring.py`（5 条注入 → 6/6）**：

| # | 注入 | 期望 |
| --- | --- | --- |
| ① | 某个 ViewModel 不再读 `page.meta.hasMore` | 数量掉下来 |
| ② | 某个界面删掉 `TruncationNote(` | 数量掉下来 |
| ③ | 另一处也开始读 `"X-Truncated"`（两套判据） | "正好 1 个"那条红 |
| ④ | 解析器失配（认不出 paged 端点） | 反空转下限红 |
| ⑤ | **页面加了、下限没同步** | "下限没过期"那条红（本轮新增的判据） |

⑤ 就是本轮那条修正的守卫：**它证明"余量"这件事现在会被机器拦住**，而不是靠谁记得去改那一行。

### 那"偶发红"的真因：这套用例是**顺序依赖**的（2026-09-25 第 52 轮）

上一轮用 `--dist loadfile` 让 CI 转绿。这一轮做**收尾验收**（全新检出连跑 4 次）时发现：
**`-n auto --dist loadfile` 4 次里红了 2 次**（每次 1~2 个失败）。抓到两条：

1. `test_export_cells_other_kinds.py::test_customers_export_matches_ledger_accounts` ——
   断言原话「导出的货主/临时货主账一行都没有：**[['批发商', 'Shipper', 15, 640]]**」：
   这位货主被导进了**批发商**那一桶。根因：`users["shipper"]` 是**每个 worker 共用**的一行，
   而批发商那几条用例把它设成 `is_member=True` **且不还原**。
2. `test_auth.py:76` —— 那条用例要注册的手机号，已经被**别的文件**建过了。

**为什么 `--dist loadfile` 挡不住**：它只保证「一个文件的用例落在同一个 worker」，而
**一个 worker 会顺序跑好几个文件**，它们**共用同一个 SQLite 库**（`conftest.get_db_path` 按 worker 取名）
→ 跨文件的假设照样踩。

**本轮的修法分两半**：

- **能力之内那一半**：`tests/conftest.py` 加一条 **autouse 夹具** —— 每个用例开始前把三个共用账号的
  可变标志摆回基线（`is_member=False` / `driver_rule_id=None` / `billing_mode=None`）。
  它能挡住"改共用账号"这一类泄漏（第 1 条就是它），**挡不住**"别人建过这个手机号"那一类（第 2 条）。
- **口径那一半（改 CI）**：**全量套件一律顺序跑** —— 去掉 `-n auto`。
  顺序跑在本机连跑多轮都是 **1015 passed**；这套用例本来就是**按文件顺序开发**的。
  代价约 **145s**（并行 33s）。⛔ **拿 2 分钟换确定性**：一个会偶发红的闸门等于没有闸门 ——
  而 CI 上那两个 job 长期红的形状，正是这个。

⚠️ **真正的修法是「每个测试文件一个库」**（或让每个用例自给自足）—— 那要重新设计 conftest 的
DB 生命周期（`SessionLocal` 被服务层直接 import，换引擎会牵连一片），是独立的一轮工作，如实记在这里。
本轮只把"闸门不再偶发红"这件事做实。

**证据**：全新检出连跑 4 次（修前 `-n auto --dist loadfile` **2 红**）；顺序跑连跑多轮 **1015 passed**；
`_check_all.py` **99/99**；`_check_ci_workflows.py` **32 条**（其反向验证第 ⑦ 条的锚点从"已改顺序"的
gate.yml 挪到"仍并行"的 test-parallel.yml 那几处 → **8/8**）。

## 收口（2026-09-25 第 53 轮）：验收页重跑全绿，仓库内的工作到此为止

**一句话**：报告里**能在仓库内做完**的每一条，都已经有产物、有一条能重跑的命令、有命令自己打印出来的结论行。
做不到的三件事**不在仓库里**，如实列在下面，不拿它们充数。

### 最终验收：18 行全绿（每行 = 报告章节 → 产物 → 复现命令 → 那命令自己打印的结论行）

重跑命令：`python _tools/qa/_gen_acceptance.py --full --out docs/RECTIFICATION_ACCEPTANCE.md`
（页面是**生成物**，⛔ 不要手改；它按"命令真的跑得通、结论行真的拿得到"判，缺一行就红）

| 报告章节 | 那一行现在打印的结论（原文摘录） |
| --- | --- |
| §2 真实世界基线 | ✅ 全部通过（改造前的快照还在，且没被改过） |
| §3 备份 / 恢复演练 | ✅ 全部通过（含恢复演练的隔离与门禁） |
| §4 Schema 迁移版本化 | 当前版本：**3**（判据 48 ≥ 18） |
| §5 CI 正式接管 | ✅ 全部通过（分支口径 / 路径 / 层序 / 危险 job 时机 / gradle 任务名都对得上） |
| §6 API 层纯搬迁 | ✅ 端点索引与源码一致（不是过期地图） |
| §7 钱：从文件冻结升级为领域契约 | ✅ 每个钱数只有一处实现，消费方真的在用它 |
| §8 状态机唯一写入口 | ✅ 全部 **55** 项通过 |
| §9 权限模型真的在执行 | ✅ **26** 个权限点都有交代（26 个在用、**0 个有书面理由**） |
| §10 事务发件箱 | ✅ 全部通过（入队同事务 / 失败可重试 / 表结构同源 / 边界可见） |
| §11 客户端大文件按职责拆 | ✅ 全部 **1280** 项通过 |
| §12 AI 能力目录与后端权限同源 | ✅ **17** 项：AI 的能力 = 角色的能力（不越权，也不缺） |
| §13 文档事实源自动化 | ✅ 活文档里手写的数字都与现状一致，或已指向生成物 |
| §15 可观测性 | ✅ 只读、阈值一处、退出码分档、该盯的都盯着 |
| §18 判据自己也要被验证 | ✅ **1146** 条注入原文全部还在（94/118 份脚本的注入表都认得出） |
| §19 Domain + Database invariants | 检查 **42** 项 / 确认缺陷 **0** |
| §5 全量静态检查 | ✅ **99/99** |
| §14 后端全量用例 | **1015 passed** |

### 仓库内：做完了

- `_check_all.py` **99/99**（本地与全新检出都验过）；
- 后端用例 **1015 passed**（顺序跑，确定性；见上一节为什么放弃并行）；
- 反向验证 **118 份脚本 / 1146 条锚点** 全部完好，最近新增的 6 份（time_base / invariants / export_guards /
  action_labels / page_truncation / ci_workflows）各自 **5~8 条注入全部当场红**；
- Android 两个 flavor 的单测都 **BUILD SUCCESSFUL**；本地迁移 **version 3**（在已有 988 行审计数据的库上应用过，重跑幂等）。

### 仓库外：三件事，都卡在同一个前提上（网络），如实列出

1. **推上去**：本地领先 `origin/new` **28 个提交**。代理能建 TCP 但到 GitHub 的 TLS 会被掐断
   （`error:0A000126:SSL routines::unexpected eof while reading`），直连 + schannel 同样失败；
   同一时刻**百度直连 200、生产机 SSH 正常** —— 所以是到 GitHub 这一段的问题，不是本机整体断网。
   纪律：判断"推没推上去"一律用 `git ls-remote origin new` 与 `git rev-parse HEAD` 对比，
   ⛔ 不看 `git status` 那行 ahead（它只说明本地领先，不说明远端收到没有），
   ⛔ 也不把 `git push` 接进管道再看（曾经因为管道吃掉退出码而误判过一次）。
2. **CI 转绿的确认**：`gate.yml` 里那两个长期红的 `Tests (Parallel)` job，本轮已按顺序跑改造，
   但**"改了"不等于"绿了"** —— 公开仓库拿不到 job 日志，只能等推上去之后读 CI 注解（`::error::` 那条通道是公开可读的）。
   按本仓库的规矩，状态栏不合并：这两条现在仍然是 **进行中**，不是已完成。
3. **生产部署**：启动时会跑迁移 002 / 003（`SORDERS_SKIP_MIGRATIONS=1` 是逃生口）。部署是用户拍板的事，本轮没做。
...
### §9 第二层的第一个域：授权从「函数体」搬到「签名上」，并当场补上一个**没人守的洞**（2026-09-25 第 56 轮）

#### 做了什么

`backend/app/api/v1/expense_categories.py` 的 5 个端点全部从 `CurrentUser` 换成
新加的签名级依赖 `DispatcherUser`（`app/deps.py`），并删掉 5 处逐字重复的
`if user_role_key(current) != UserRole.DISPATCHER.value: raise HTTPException(403, ...)`。

这不是"换个写法"，它有机器可读的后果 —— 端点索引里那 5 行的**授权列**变了：

| | 改前 | 改后 |
| --- | --- | --- |
| 授权列 | `仅登录 + 体内仅允许:派单员` | `角色:dispatcher` |

改前那一列的意思是"**只看得到你登录了**，至于谁能调，得自己去读函数体"。
AI 读能力裁剪（`_gen_ai_read_catalog.py`）就是靠读这一列 + 体内 raise 来判"这个角色能不能读这张表"——
体内门槛越多，它越只能靠猜；裁多了是能力缺失，裁少了是越权。

#### 本轮真正值得记的：一个**没有任何判据守着**的地方（实证，不是推测）

按纪律，改完一条"收敛"必须配反向验证。于是先问：**如果体内门槛又长回来，谁抓得住？**
grep 下来只有 `_tools/qa/_check_permission_model.py` 在看角色/权限，而它的 8 条判据全部只针对
`rbac.role_has_permission` 的**函数体内部**（Scope 表、例外角色表、不许硬编码角色名）。

那就实测：往 `expense_categories.py` 一个端点体内**加回**一段真实的角色门槛（不动签名，
所以端点索引不会因此变化），然后跑检查。结果：

```
权限模型：26 个权限点 / 26 条 Scope / 1 条例外角色
  ✅ 8 组判据全部通过
```

**8/8 全绿** —— 当时全库 102 个检查里，没有任何一条抓得住"授权又回到函数体里"。
（`_check_all.py` 确实报了 3 处红，但逐条核对全是注入本身导致的产物过期／后端跑旧代码，
与角色门槛无关 —— 这正是"红了不代表判据抓到了"的典型，得逐条读。）

#### 补的判据：`_tools/qa/_check_inline_role_gates.py`（认代码形状，不认文字）

用 AST 扫 `backend/app/api/**/*.py` 里被 `@router.<verb>` 装饰的处理函数体，
把**由角色/权限比较决定**的 `raise HTTPException(401/403)` 记一处。5 组判据：

1. **棘轮**：总数 ≤ 台账最后一条（当前 **44 处 / 13 个文件**），只许降不许升；
2. **已收敛文件必须归零**：`CONVERTED` 表里的文件（现在是 `expense_categories.py`）体内必须是 0 —— 防"改完又长回来"；
3. **清单防化石**：`CONVERTED` 里的路径必须存在；
4. **台账非增 + 有理由**：`HISTORY` 逐条计数不许上升；不减的那条必须写「理由:」与说明（≥12 字）；
5. **扫描不许空转**：路由处理函数少于 60 个（实测 225）就报红 ——「算不出真值＝红」。

**为什么是棘轮而不是"必须为 0"**：有一类门槛**依赖请求内容**，签名级表达不了，例如
`GET /users/{id}`（要么是派单员、要么是你自己）与 `GET /orders?deleted_only=`（这个参数只有派单员能用）。
一刀切要求"全是签名级"会逼着人把这类门槛写成更糟的形状。所以定成棘轮：一轮收敛一个域，数字跟着降。

#### 反向验证：`_tools/qa/_reverse_verify_inline_role_gates.py` → **10/10**

| # | 注入 | 期望 | 结果 |
| --- | --- | --- | --- |
| ① | 已收敛文件里加回一段体内门槛 | 红：声明已收敛 | ✅ |
| ② | 别的文件里新增一处体内门槛 | 红：涨到 45 处（超棘轮） | ✅ |
| ③ | 台账追加一条**增大且没写理由**的记录 | 红：没写「理由:」 | ✅ |
| ④ | 台账追加一条增大**并写明理由**的记录 | **绿**（如实承认这是被允许的松绑路径） | ✅ |
| ⑤ | 清单路径改成不存在的文件 | 红：清单化石 | ✅ |
| ⑥ | API 目录指到不存在的地方 | 红：只扫到 0 个 | ✅ |
| ⑦ | 台账说明缩成两个字 | 红：说明太短 | ✅ |
| ⑧ | 加一段**不 raise** 的角色分支（数据范围裁剪） | **绿**（不许误报） | ✅ |
| ⑨ | 加一个**不由角色决定**的 403 | **绿**（不许误报） | ✅ |

⑧⑨ 两条是**必须仍然绿**的防误报用例：一条判据如果把"司机只看自己的单"也算成门槛，
它就会变成噪音，最后被谁顺手关掉 —— 那比没有判据更糟。

#### 如实说明：这条判据**挡不住什么**

有意**同时**改台账与代码的人能绕过去 —— 台账是自证的，"把上限调大"这件事在代码里没有独立事实可对质。
它挡的是真实发生的那一类：**顺手又写了一处体内门槛、谁也没注意到**，以及改完又长回来。
同时改两处的动作会出现在 diff 里，由人看出来（这也正是台账要写日期与说明的原因）。
这条限制写进了脚本自己的 docstring，不假装它无懈可击。

#### 数字

- 体内角色门槛：**44 处 / 13 个文件**（棘轮起始，`expense_categories.py` 已归零）；
- `python _tools/qa/_check_all.py` → **103/103**（新增 1 个检查脚本，清单自己算）；
- 后端全量 **1015 passed**（114.04s）；反向验证 **10/10**。
...
### §14 第二次真跑：KVM 修好了、模拟器 1.3 分钟起来了 —— 然后它**第一次真的红了**（2026-09-25 第 58 轮）

#### 先把两件事分开说

| | 第一次（`b8a39e0`） | 这一次（`d6ba699`） |
| --- | --- | --- |
| 硬件加速 | `disable Linux hardware acceleration: true` | **false** ← KVM 生效了 |
| 模拟器 | 16 分钟没启动完，超时 | **约 1.3 分钟起来了**（日志出现 Increasing screen off timeout 之后就是脚本） |
| 作业耗时 | 23.7 分钟 | **7.8 分钟** |
| 作业结论 | **success**（而日志说「这次没有跑」） | **failure**（真的跑了、真的挂了） |

「开 KVM 那条 udev 规则」这一步是对的：日志里 `disable Linux hardware acceleration` 从 true 变成 false，
模拟器正常启动。**上一步的修法成立。**
...
`~
##[warning]这台 runner 上没能起模拟器 —— **这次没有跑**，不是通过。
##[error]连 /tmp/e2e.rc 都没有 —— 模拟器压根没起来（先看上面有没有 ProbeKVM / boot timeout）。
##[error]Process completed with exit code 1.
`~

第 57 轮给「没跑成」那一支改的 `exit 1` 当场起作用：**「没跑」不再被算成「通过」**。
（其实这一次它**跑了**，只是脚本自己退 2 —— 判定步骤分不出这两者，见下面「已知的粗糙处」。）

#### 真因：那个 action 把 `script` **按行**执行

日志里这两行是**两次独立的调用**（不是一次多行命令）：

`~
[command]/usr/bin/sh -c set +e
[command]/usr/bin/sh -c python _tools/e2e/_flow_login_nav_order.py --check-env --serial 5554 \
`~

于是多行脚本里有三样东西**静默失效**：

1. `set +e` 管不到后面（它自己就是一个进程，跑完就没了）；
2. 变量活不过一行 —— `rc=$?` 下一行读到的是空的；
3. **行尾反斜杠变成一个真实参数** → 脚本直接 `error: unrecognized arguments` 退 2。

三种都不报「你的写法不对」，只在**真跑**时才炸 —— 而这正是「接了线但不真跑」的典型后果：
在此之前它从来没跑过，所以这个 bug 从来没暴露过。

**修法**：整条流程压成**一行**（分号 / `||` 连接），并加判据第 16 条钉住「只能一行」。

#### 判据与反向验证

- `_check_ci_workflows.py` 第 16 条：模拟器 action 的 `with.script` **必须只有一行**；
  `MIN_RULES` 38 → **39**（贴着实际值）；
- `_reverse_verify_ci_workflows.py` 第 ⑯ 条：把 script 拆回多行 → 必须红 → **16/16 全部成立**；
- 顺带：`_check_ledger_cash.py` 的两条判据随 §9 第二域一起从「体内文字」搬到「签名」，
  反向验证 17/17。

#### 已知的粗糙处（如实记，下一轮再说）

判定步骤现在分不出「模拟器没起来」与「脚本自己退 2」—— 两者都会走到「没有 rc 文件」那一支，
因为**写 rc 的那一行本身**没跑成。这一轮靠翻日志才分清。下一轮可以让脚本第一件事就写一个
「跑到了」的哨兵值，失败时再覆盖成真实 rc。

⏭️ 下一次推送才是真正的验收：一行版脚本在 CI 上跑通（或者给出**流程本身**的失败原因）。

### §14 第三次真跑：又一块「永远绿的装饰牌子」被拆掉 —— 端到端从落地起就不可能跑过（2026-09-25 第 59 轮）

#### 结果：Gate 5/5 全绿，而端到端走的是「带理由跳过」

`Gate @ 9f2e8a1` 整体 **success**、端到端 8.6 分钟 success。按上一轮立的规矩，**先翻日志再下结论**：

`~
SKIP: emulator-5554 上没装 com.tapmoay.sorders —— 先装包：python _tools/qa/_install_all.py --only 5554
##[warning]SKIP: emulator-5554 上没装 com.tapmoay.sorders
`~

KVM 之后模拟器 1.3 分钟就起来了（不是"没跑"），但流程**压根跑不到**。

#### 第三个结构性 bug

`--check-env` 体检四样：adb / 设备 / **App 装没装** / 后端。
而 gate.yml 里的顺序是**先体检、后装包** —— 于是它必然报「没装 App」并 rc=3 跳过。

**也就是说：`ef6854b` 把这条端到端接进 CI 的那一天起，它就不可能真的跑过流程。**
本轮一共拆掉三块这样的牌子（每一块都显示 success）：

| # | 牌子 | 表现 | 靠什么发现 |
| --- | --- | --- | --- |
| ① | KVM 没开 | 模拟器 16 分钟没起来 | 第 57 轮把「没跑」改成红 |
| ② | 那个 action 逐行执行 script | 脚本退 2（行尾反斜杠成了参数） | 第 57 轮把「没跑」改成红 |
| ③ | 体检排在装包前 | rc=3 跳过，永远跑不到流程 | 第 58 轮加的哨兵 + 翻日志 |

#### 修法

- 一行版脚本改成 **先 `adb install`、再 `--check-env`、最后跑流程**；
- 判据第 18 条：`adb install` 必须出现在第一次 `--check-env` **之前**；`MIN_RULES` 40 → 41；
- 反向验证第 ⑱ 条：把体检挪回装包前面 → 必须红；
- 顺带把 ⑯/⑰ 两条 RV 的锚点改成**短且不受行内顺序影响**的形态（这一行一轮里被改过三次，
  长锚点每次都被打成 0 次命中 → RV 报 SKIP → 记成 MISS）。

#### 同一轮的另一件事：§9 第二层又收敛两个域

- **第三域（expenses）**：两个无条件「只有派单员」的端点 → 签名级 `DispatcherUser`；
- **第四域（products）**：读门从体内 `role_has_permission` 换成签名上的
  `require_permission(Permission.ORDER_CREATE)` —— 这一道是**权限点**门槛，正是报告 §9 说的
  「require_permission 成为唯一入口」；行级过滤原样留在体内（两件事各管一段）；
- **不转的那一域（notifications）**：三处是「要么是你自己的、要么你是派单员」+ **404**，
  依赖具体那条记录，签名级表达不了 —— 留在体内，台账上记着。这就是棘轮存在的理由。

棘轮：44 → 41 → 39 → **38 处 / 10 个文件**。

⏭️ 下一次推送是第一回「脚本按正确顺序真的跑到流程」—— 那才是这条端到端的真正验收。

### §14 第四~六个真 bug 连着拆：连接 → 权限 → launcher ANR（2026-09-25 第 60 轮）

#### 一张表看完这一串（每次都是「红了 → 修 → 又前进一段」）

| 轮次 | 真跑的结果 | 卡在哪 | 修法 |
| --- | --- | --- | --- |
| 第 57 轮 | success（其实没跑） | 模拟器 16 分钟没起来（KVM 没开） | 加 udev 规则开 KVM |
| 第 58 轮 | failure | 那个 action **逐行**执行 `script`，行尾反斜杠成了参数 | 压成一整行 + 哨兵 rc |
| 第 59 轮 a | success（其实没跑） | `--check-env` 排在 `adb install` 之前 → 恒 SKIP | 先装包再体检 |
| 第 59 轮 b | failure | 后端绑 127.0.0.1，App 连不上 10.0.2.2 | 改绑 0.0.0.0 |
| 第 59 轮 c | failure | 全新安装弹出系统权限窗挡住工作台 | 跑流程前预授四个运行时权限 |
| 第 60 轮 | failure | `Pixel Launcher isn't responding`（模拟器被拖到 ANR） | 4 核/4G + 装完先安定 20 秒 |

两次「success（其实没跑）」是同一类：**作业绿着，日志却在说没跑**。
把它们变红的那个改动（第 57 轮把「没跑成」从 `exit 0` 改成 `exit 1`）是这一串的起点 ——
后面五个 bug 都是**被它逼出来的**。

#### 值得单独记的两条判断

1. **「runner 连得上」证明不了「模拟器里的 App 连得上」**。
   同一次体检里写着 `后端 可登录`（runner 自己 curl 127.0.0.1 打通的），而模拟器里的 App
   报 `Failed to connect to /10.0.2.2:8000` —— 两个网络命名空间。原来那句注释
   「绑 loopback 就够」是被**实测**推翻的，已换成实测记录。
2. **「本地永远复现不出来」的阻塞，正是端到端要跑在干净环境里的理由**。
   权限弹窗在开发机上早被手点过；只有 CI 的**全新安装**才会撞上。

#### 同一轮的 §9 第五域

`orders_lifecycle.py` 里 2 处**无条件**门收到签名上（双角色 `require_roles(SHIPPER, DISPATCHER)`
＋纯派单员 `DispatcherUser`）；同文件另 2 处**故意不转**：
货主专属的权限点门（嵌在 `if role == SHIPPER` 分支里，签名级表达不了）、
角色链的 `else` 兜底（转了会变死代码）。所以它**不进 CONVERTED 表** —— 那是刻意留的口子。

棘轮：44 → 41 → 39 → 38 → **36 处**。

### §14 收官：**四段全通**（2026-09-25 第 61 轮）

`~
adb=…｜emulator-5554｜App 已装｜后端 可登录
✅ 环境齐，可以跑
登录      ✅ 登录后进了工作台
...
**`Gate @ 38f8ff3` 整轮 success、`Tests (Parallel)` success**（这个 job 之前在六轮里一直是红的）。

#### 这七轮到底在修什么（一张表收口）

| # | 真 bug | 为什么之前看不出来 |
| --- | --- | --- |
| ① | runner 上 KVM 没开 → 模拟器 16 分钟起不来 | 作业显示 **success**（「没跑」被算成通过） |
| ② | 那个 action **逐行**执行 `script` → 行尾反斜杠成了实参、`set`/变量全失效 | 同上 |
| ③ | `--check-env` 排在 `adb install` 之前 → 恒 SKIP | 作业又是 **success** |
| ④ | 后端绑 `127.0.0.1` → App 连不上 `10.0.2.2` | 体检那一步是绿的（两个网络命名空间） |
| ⑤ | 全新安装的系统权限弹窗挡住工作台 | 开发机上早被手点过，**本地复现不出来** |
| ⑥ | launcher 被拖到 ANR（2 核 + 软件渲染） | 只在资源紧张的 CI 上出现 |
| ⑦ | CI 里没有商品/地址数据 | 「下单」段才需要它们，而前两段仍然全绿 |

⛔ **转折点是第 57 轮那一个字符**：把「没跑成」从 `exit 0` 改成 `exit 1`。
在它之前，① ② ③ 全都以 **success** 的样子挂着；在它之后，每一轮都**红着说出下一个真问题**。
七个 bug 里有三个是"作业绿着、日志说没跑"——这一条判据就是它们的总开关。

#### 配套的判据（这一串没白修：每条都有反向验证钉着）

`_check_ci_workflows.py` 第 14~21 条 + 反向验证第 ⑩~㉑ 条（**21/21**）：
「没跑成要红」「跑挂要红」「带理由跳过要可见」「script 只能一行」「先写哨兵」
「先装包再体检」「后端绑 0.0.0.0」「跑流程前预授权限」「要播业务数据」——
每一条都是**实测踩出来的**，不是照着想象写的。

#### 顺带记一条与排查有关的事实
第 61 轮那次 rerun 是**基础设施**问题（`actions/setup-java` 报 Maven 仓库不可达），
与本次改动无关；用 API 重跑失败 job 即可（`POST /actions/runs/{id}/rerun-failed-jobs`）。
⚠️ 顺带记下来：**不要因为一次红就改代码** —— 先分清"这次红的是我们的东西还是 runner 的"。
