...
| 4 | §6 | `reports.py` 聚合下沉到 service | 否 | **拆成 3 步做**（试了 3 次、每次都按纪律回退 —— 批量改判据/锚点必然留红；顺序：① 只改判据读取口径（5 处读点 + `_check_single_source` 的调用点规则 + `_airepo.reports_source()`，且 **service 文件不存在时也要能跑**）；② 再下沉；③ **逐个**反向验证脚本重指锚点，一次一个、改完立刻跑 —— 本轮批量重指把 8 条改错、13 条锚点失效）：下沉本身**契约零差异**（`after-reports-sink` 快照），但 5 条判据是**按文件文本**读 `api/v1/reports.py` 找聚合锚点的（`_check_ai_guardrails` / `_check_cost_basis` / `_check_order_return` / `_check_report_window` / `_check_single_source`）—— 下沉前要先给它们加"读两份（api + service）"的口径，与 orders 那套 `orders_api_source` 同形 |
