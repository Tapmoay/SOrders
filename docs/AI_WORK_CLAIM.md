...
### [2026-09-24 16:0x → 18:3x] 会话：**全项目系统性复核 · 第 35 轮**（还第 34 轮欠的定向用例：R11-8；**顺手抓到一条工具自身的假红 + 一次注入残留事故**）【进行中】（DSH `session-78ebd95c-b8c9-4a44-8f7a-270d17e7c918`）
...
| 步骤 | 在哪做的 | 库内证据 |
|---|---|---|
| 建单 → 派单(收现金) → 司机收现金送达 | API 准备（前一轮已验真机链路） | `SO202609234752051112`：`paid=true / settled=69.60` |
| **申请退货**（默认全部勾满 → 提交） | **5556 货主真机**（「申请退货」→ 2 件 → 提交申请） | `order_return_requests#13 status=pending`；**订单/账本/库存一个字都没变** ✓（弹层那句"这只是申请：提交后库存和账本都不会变"是真的）；派单员收到 `order.return_request` 站内信 ✓ |
| **办理退货**（数量锁死、不能改） | **5554 派单员真机**（退货申请 → 「办理退货」→ 「确认退货」） | 订单 → `RETURNED` + `returned_at`；行 `returned_quantity=2`；账本 +69.6 / **RETURN −69.6（成本快照 −52）**；库存 **+2 回库（status=RETURNED）**；**退现流水 OUT 69.60 `REFUND_CUSTOMER`**；货主收到 `order.return_request.done` ✓ |

钱的口径逐项对上：`订单金额 − 已退 = (已收 − 已退现) + 欠款` → `69.60 − 69.60 = (69.60 − 69.60) + 0` ✓；
司机账单**不动**（22.00 仍 open）—— 那是设计如此（"不还原司机账单"写在服务文档里）。
截图：`_agent/e2e/e2e-return-request-pending.png` 等。

**二、真机抓到的缺陷：退现那一行的「对象」写成了「临时货主」**

退现流水 `party_name='临时货主'`，而同一张单在账本/订单页上写着「永盛食品」—— **同一笔钱两个名字**。
根因：`order_return` 那行 `party_name = cust.name if cust else (… or "临时货主")`，
而 `cust` 查的是**客户档案行**，**注册货主不一定有档案**（生产 34 个货主账号里 32 个有 → 那 2 个的单
一旦退现就会写错人；本机开发库的货主 2 就没有档案，所以真机一退就现形）。
⚠️ 这条规矩**本仓库早就写过**（`services/ledger_response._shipper_name`：「宁可给个编号，
也不要给一个"临时货主"——那会把**注册货主的账说成临时货主的**」）—— 只是那份用在**账本出参**上，
退款流水这一处漏了：**同一件事两份实现、只改了其中一份**。
生产证据（只读）：退款流水 0 行（还没退过现）→ 缺陷是**潜伏**的；`ledgers.customer_id` 4570/4570 全空
（另一条线，见下）。

- 核心改动：`backend/app/services/order_return.py` —— 为什么必须动核心：它是**退货这条钱链路**的
  唯一实现（红冲 + 回库 + 退现），而退现那一行的"对象是谁"原来自己抄了一份兜底：
  注册货主没有客户档案时会把账说成临时货主的（同一笔钱两个名字）。

**修法**：把"这一单的货主叫什么"收成**一份**——`services/ledger_response.order_shipper_label(db, order)`
（与账本出参 `_shipper_name` 同一条规矩：注册货主 → 人名/手机号/`货主#编号`；临时货主 → 那句称呼），
`order_return` 的退现行改调它。⛔ 「临时货主」这个兜底只保留在**真的没有账号**的那一档。

**三、判据**
- 单测 `backend/tests/test_refund_party_name.py`（2 例）：**注册货主 + 没有客户档案** → 退现对象名是
  那个人的名字（且与订单出参同源）；**临时货主** → 用它那句称呼（兜底仍然在）；
- 红线 `_check_order_return.py` **114 → 117 项**：退现对象名必须走共用口径、退现那一块**不许**再出现
  「临时货主」兜底、共用实现必须还在；反向验证 **29 → 31 条注入**（含"把兜底改回去"与"共用函数改名"）；
- 生产只读探针新增不变式 **AA**（退现流水对象名写成「临时货主」而订单是注册货主的 = 0 行）；
  体检总数 26 → **27 条**。

**四、顺带发现（记进台账，没擅自改）**：生产 `ledgers.customer_id` **4570/4570 全为 NULL**
（ORDER 账本行一个客户归属都没填）。客户账聚合走的是 `shipper_id`/临时名，所以现在不影响口径；
但"客户合并/客户账"那条线要用 `customer_id` 时会发现它一直是空的 —— 已记 FINDINGS 待拍板第 20 条。

**要改的文件**：`backend/app/services/order_return.py`、`backend/app/services/ledger_response.py`、
`backend/tests/test_refund_party_name.py`(新)、`_tools/qa/{_check_order_return,_reverse_verify_order_return,_probe_prod_readonly}.py`、
定位表、台账。

**验收数字**：`_check_all.py` **85/85** · 后端 `pytest` **823 passed**（+2）· 退货红线 **117 项** +
反向验证 **31/31** · 生产只读体检 **24/27 条有发言权且 0 违规** · 真机三设备全链路截图与库内逐项对账。
...
**一、先把「承诺有没有被执行」拿生产证据量了一遍（只读）**

生产 journal + 库 + 磁盘三处对账：`journalctl` 里有 **24 条**「数据保留治理完成」，最近一条 11 小时前，
带真实删除量（`notifications_purged: 24`）；站内信最老一条 2026-08-24 01:01 UTC，而今天 00:58 UTC 跑过一轮
（截止线 08-24 00:58）→ **正好卡在「保留 30 天 + 不到 1 个运行周期」**，说明每日循环在正常工作。
⚠️ 这条判据不能写成"最老 ≤ 30 天"：每日任务在两次运行之间天然会留下"刚过期还没轮到"的行，
写死 30 天会天天假红 —— 口径是**保留期 + 一个运行周期**。

**二、把"指向 orders 的外键"逐个盘了一遍 → 抓到一条静默的重缺陷**

`orders` 上有 **9 个外键**，而 `data_retention.delete_orders_by_ids` 原来只解开了 5 个。
漏掉的四个里，`orders.parent_order_id` 是自引用（置空即可），另外三个是 **NOT NULL**：
`order_return_requests.order_id`（退货申请）、`shipper_settlements.order_id` / `shipper_settlement_lines.order_id`（货主核销凭证）。

⚠️ 后果**不是"这一单删不掉"，而是"当天的治理全部停摆"**：异常一路冒到 `main._retention_sync`
被吞成一行日志 + `db.rollback()` → 3 年清理、消息清理、图片归档、导出产物清理**全不执行，且每天重复失败**
（2026-09-19 审计 R12 的同一形状，当时是 `places.first_order_id`）。而 pytest 里**一条都没覆盖它** —— 测试语料里就没有"带退货申请/核销凭证的软删单"。

**修法（三条，都不许毁凭证）**
1. **`_orders_with_blocking_docs`**：还挂着退货申请 / 核销凭证的单**不做物理清理** ——
   写日志 + **给派单员发站内信**（钱的事必须有人能在界面上看见，同 R12-H4 给司机账单定的口径），
   其余单照删。⛔ 不删凭证：核销单是"这笔钱收到了"的唯一记录，删了就复核不了；
2. **子单的 `parent_order_id` 置空**（与 `places.first_order_id` 同一个手法）；
3. **每一步各一个 savepoint**：一步失败只丢那一步（返回值标 `-1`），不再让整套治理一起回滚 ——
   "一张单卡住就让当天所有清理停摆"这件事从结构上消掉。

**三、判据（三层，全绿才算）**
| 层 | 位置 | 钉什么 |
|---|---|---|
| 结构 | `_tools/qa/_check_order_purge_fk.py`（25 项，新） | 外键清单**从 metadata 算**（orders.id + order_products.id，12 个）；处理方式**从 AST 算**（delete/update 置空）；每个外键要么删、要么置空、要么"不许删 + 书面理由"；`BLOCKED_BY` 的理由必须真的被代码查了、而且那些表必须**还真的指着订单**（防化石）；没有外键但指着订单的列（`cash_flows` / `expenses` / 两个 JSON 列）逐个写"为什么允许悬空" |
| 行为 | `backend/tests/test_order_purge_foreign_keys.py`（4 例，新） | 带退货申请 / 核销凭证的单**不删**且发站内信；父单被清后子单的指向被解开；**一步失败不拖垮其余**（返回值里标 -1） |
| 真机 | `_probe_prod_readonly.py`（26 条不变式 + ⑦ 节） | 「保留承诺」四条（站内信 30+1 天 / 隔离区 30+1 天 / 3 年 / 悬空引用）+ journal 里真的有治理记录 + 导出目录与图片目录的超期文件 |

**反向验证 9/9**（`_tools/qa/_reverse_verify_order_purge_fk.py`）：去掉核销凭证的挡 / 去掉退货申请的挡 /
不再置空 `parent_order_id` / 不再置空 `first_order_id` / **新加一张指着 orders 的表而清理没提它** /
理由表塞化石 / **行为注入**（挡住清单改成永远返回空 → 由单测抓）/ 清理不再调用挡住清单 / 外键清单盘空。

**四、顺带把 `data_retention.py` 登记进核心区**（`_tools/qa/_core_files.txt`）：
它是"历史数据变样"的唯一入口，动它必须留声明 —— 与 `schema_bootstrap.py` 同理。

- 核心改动：`backend/app/services/data_retention.py` —— 为什么必须动核心：它是**历史数据变样的唯一入口**
  （物理删单会连带删账本/日志/库存流水/图片），而这一轮修的是"漏解外键 → 当天整套治理停摆"。

**要改的文件**：`backend/app/services/data_retention.py`、`backend/tests/test_order_purge_foreign_keys.py`(新)、
`_tools/qa/{_check_order_purge_fk,_reverse_verify_order_purge_fk}.py`(新)、`_tools/qa/_probe_prod_readonly.py`、
`_tools/qa/_core_files.txt`、定位表、台账。

**验收数字**：`_check_all.py` **84/84**（+1 脚本）· 后端 `pytest` **821 passed**（+4）·
外键红线 **25 项** + 反向验证 **9/9** · 生产只读体检 **24/26 条有发言权且 0 违规**（2 条数据还不够）。

### [2026-09-23 19:4x → 20:2x] 会话：**全项目系统性复核 · 第 12 轮**（导出逐格对账补齐**另外四个 kind**：资金收支 / 客户经营 / 司机绩效 / 异常与审计 —— 六个 kind 全覆盖，13 条注入）**【已完成】**（DSH `session-78ebd95c-b8c9-4a44-8f7a-270d17e7c918`）

第 11 轮把 `turnover` / `products` 逐格钉住，这一轮补齐剩下四个 —— 它们各有各的"两处口径"风险，
而且**分叉了不会有任何接口报错**：

| kind | 钉的是什么 |
|---|---|
| `finance` 资金收支 | 流入/流出/净额是**在内存里按行求和**的（不走 `/cash-flows/summary`）→ 漏一个 `is_deleted` 过滤就等于把**已撤销的付款**又算了一遍；净额正负号写反也看不出来 |
| `customers` 客户经营 | 导出**自己写了一份**客户账聚合（不是复用 `/ledger/accounts`）→ 隔离区（软删）单的账算不算、窗口怎么下推，两处一旦不一致就是两个数（R13-R6） |
| `drivers` 司机绩效 | 「待结运费」必须是**数字**（文本在 Excel 里 SUM 得 0）、工资制那一格必须是**文字「工资制」**（印 0 会被读成"这个月一分钱都不用付"） |
| `audit` 异常与审计 | 「敏感操作日志」曾经**完全不看区间**；截断说明必须如实（"共 N 条" / "只列了最近 M 条"） |

**造现场（每条判据都要有发言权，不然就是恒真）**
- 供应商 → 应付 → 付一笔款 → **`DELETE /supplier-payments/{id}` 撤销它**（`cash_flows` 的软删唯一写入点）
  → 接口与导出的「流出」都必须扣掉它；
- 送达一张单再 `DELETE /orders/{id}` 进回收站 → 它那几行账本还在库里，但两个口径都**不许算**；
- 给探针司机挂「每单固定 100」的计费规则（待结运费 > 0 → 数字那一支有人覆盖），
  另建一个 `billing_mode=SALARY` 的司机（文字「工资制」那一支）；
- ⚠️ **直接插一条三天前的操作日志**：测试库里的日志全是"今天"的，否则"日志块不看区间"这件事
  根本验不出来（把区间放宽十年也不会有别天的行 —— 反向验证实测就是这么漏掉的）。

**反向验证 7 → 13 条注入**（新增：撤销的流水又算进来 / 净额符号写反 / 流水金额写成文本 /
客户经营不过滤隔离区 / 待结运费写成文本 / 审计日志块不看区间），13/13 全被抓到。

**顺带修掉三处"判据自己空转/假红"**（都是全量跑才暴露的）：
① 固定列的表用"有值才取"的助手读 → 某一格是空串时那一行比表头短，按下标取值 IndexError
（改成**按列号**取）；② 空值按 `==` 比 → 导出写 `None`、接口给 `""`（在 Excel 与界面上都是空白，
不是分叉），现在统一按空串比；③ 客户经营只比了「货主」那一段，而 `/ledger/accounts?kind=shipper`
**同时包含真货主与临时货主两种桶** → 被别的用例留下的临时货主打成假红，现在按
「货主+临时货主 ↔ kind=shipper」「批发商 ↔ kind=member」两两对上。

**本轮没有动任何生产代码**（核心区零改动）：只加判据与文档。

**要改的文件**：`backend/tests/test_export_cells_other_kinds.py`(新)、
`backend/tests/test_export_cells_match_api.py`(助手按列号取 + 清理补现金流水)、
`_tools/qa/_reverse_verify_export_cells.py`(7→13)、定位表、05_TESTING、台账。

**验收数字**：`_check_all.py` **83/83** · 后端 `pytest` **817 passed**（+4）·
导出逐格反向验证 **13/13** · 六个 kind 逐格 == 接口全绿。

### [2026-09-23 19:0x → 19:3x] 会话：**全项目系统性复核 · 第 11 轮**（**导出 xlsx 逐格对账**：导出是给外部看的凭证，而它与页面走两条完全不同的序列化路径 —— 立判据 + 7 条注入，顺带抓到两个"判据自己空转"的坑）**【已完成】**（DSH `session-78ebd95c-b8c9-4a44-8f7a-270d17e7c918`）

第 10 轮修枚举口径，这一轮查**出口**：同一段窗口，页面（JSON）与导出（xlsx）必须逐格一致。

**一、为什么导出要单独钉**

页面走 pydantic 出参，导出走 `ws.append([...])` 一列一列铺。两边分叉**不会有任何接口报错**：
标签顺序写反（表里「金额」那列其实是运费）、文件名按锚点日而内容按整月（R2-4 那一类"),
算不出成本的行印成数字 0（第十七轮修掉的老缺陷又回来）、金额写成文本（Excel 里 SUM 得 0）、
商品行少写一格（后面所有列整体错位）。用户拿这份表对账/存档/发给别人 —— 错了没人会发现。

**二、新判据**：`backend/tests/test_export_cells_match_api.py`（2 例）
真实链路造一张当日已送达的单（**一行有成本、一行没成本** → 商品页必须出现一格 `—`），
同窗口分别打 `/reports/turnover`、`/reports/products`（JSON）与 `/reports/export`（xlsx），
**逐格比对**：文件名 == 真实区间、表头窗口标签 == 请求那一段、营业纵览 5 行每格 == 接口字段、
毛利按 `cost_covered_amount − cost_total`（两侧同批行）、曲线逐行、商品行逐行（含
「算不出成本 → 必须写 `—`」）、金额列必须是**数**不是文本；跨日那例用近 7 天窗口，
**行数从表里数出来**。

**三、反向验证 7 条**：`_tools/qa/_reverse_verify_export_cells.py`（注入后跑那个用例文件，
要求必须 `failed`，不接受"收集期错误"）。⚠️ 第一版漏了一条（"曲线只写第一行"照样全绿），
两个原因：① 用例当时按**接口长度**去读表（`range(1, len(series)+1)`）→ 少写几行永远看不出来；
② 单日窗口曲线只有 1 个点，截断不可见。修法：行数从表里数 + 加一条近 7 天窗口的用例 +
断言点数 ≥ 2。另外顺带踩到舍入坑：接口 `12.3450`、导出 `ROUND_HALF_UP` 给 12.35，
而 `Decimal.quantize` **默认 ROUND_HALF_EVEN** 给 12.34 → 对账助手现在显式 `ROUND_HALF_UP`。

**四、AI 读能力例行复核**：`_tools/ai/_probe_read_roles.py` 打本机真后端，
**36 张表 × 3 角色的读权限与实测一致**（403 = 真的不给）。

**五、一条待拍板**（没擅自改）：导出表头那段区间标签只有"整月"带年份（`2026-09月`），
其余是 `9-1~9-23`、单日 `9-18` —— 文件名是全的，但把表头单独截图/打印时期间不能自证。
记在台账「待拍板」第 18 条。

**本轮没有动任何生产代码**（核心区零改动）：只加判据（新测试 + 新反向验证脚本）。

**要改的文件**：`backend/tests/test_export_cells_match_api.py`(新)、
`_tools/qa/_reverse_verify_export_cells.py`(新)、`docs/PROJECT_MAP/{05_TESTING,08_CODE_LOCATOR}.md`、
`_archive/audit/FINDINGS.md`(gitignore)。

**验收数字**：`_check_all.py` **83/83** · 后端 `pytest` **813 passed**（+2）·
导出逐格反向验证 **7/7** · AI 读能力对账 **36 表 × 3 角色全一致**。
...
### [2026-09-23 09:5x → 10:2x] 会话：**选品页「添加商品」的数量小窗改版**（单位只读上标题最右 / 数字居中 / 步进器收成一份）【已完成】（DSH `session-83da1ad7-e539-4d60-9412-b46a9a9dc48e`）

**用户原话**（对着选品页「＋」弹出的那个小窗，还在标题右边圈了个红框）：
> 「…**放在最右边**…那个**显示单位**也就这个商品的单位，然后下面是**数量**…那个**中间的数字**把它改成**居中**…还有那个 **+- 的样式和图标样式也全部改一下**，**整体的样貌全都发生改变一下**…非常的不顺眼啊，非常的难受」

**根因（不是审美问题，是"两份实现"）**：「`−` + 数量框 + `+`」这一组在两个弹窗里**各写了一遍**，
而且长得不一样 —— 选品页是「淡蓝实心圆 + 92dp 框」、下单页行编辑是「**默认色的灰紫**实心圆 + 96dp 框」，
**两边数字都贴左**（那是 `textAlign` 的缺省值，没人写错、只是没人写）；数量判据（`intInput(v,4)…coerceIn(1,9999) ?: 1`）
也是**同一串抄了两遍**。而 `OrderCreateScreen.kt` 里那句注释写着「全 App 同一个形态，见 `ProductPicker::QtyDialog`」
—— **注释说的是愿望，代码是两份**。单位则压根没出现在弹窗里（用户圈的那块地是空的）。

**改了什么**
1. **新 `ui/common/QtyStepper.kt`**（唯一实现处）：`QtyStepper`（一组三格：一个圆角描边包住，`−` / 数量 / `+`，
   两条 `outlineVariant` 竖分隔线，中间的框把自带描边让开）+ `typedQty`（数量判据，含上下限常量）
   + `UnitTag`（单位**只读**标签）。图标换 `Icons.Rounded.Add/Remove`；禁用那一格转灰但**照样画出来**。
2. `ui/common/ProductPicker.kt`：`QtyDialog` 标题行改成「商品名（`weight(1f)`）+ 单位标签贴最右」，
   数量行用 `QtyStepper`（`weight(1f)` 拉伸 → 数字框永远在正中间），小计改成「标签灰字 + 金额 `MoneyOrange` 加粗」；
   删掉了自己那份 `StepButton`。文件头那段「为什么小窗里没有单位了」**改口径**为「只显示、不给改」。
3. `ui/shipper/OrderCreateScreen.kt`：`LineEditDialog` 用同一个 `QtyStepper`；标题行也加只读单位标签；
   小计从「一行左对齐、`primary` 色」改成「标签灰字 + 金额 `MoneyOrange`」（钱的语义色全 App 同色）。
4. **单测** `test/.../ui/common/QtyStepperTest.kt`（8 例：正常/空串/0/字母符号/全角归一/超四位截断/负数/常量自洽）。
5. **新红线** `_tools/qa/_check_qty_dialog_style.py`（**22 项**）+ **反向验证** `_tools/qa/_reverse_verify_qty_dialog.py`（**9 条注入全被抓**）。
6. 文档：`06_DESIGN_SYSTEM.md` 新增 **§4.21**（含"第三处数量加减不在这里"的口径说明）、定位表那一行改口径、
   `09A_HINT_CATALOG.md` 重生成（多了 1 个 `.kt` 文件 → 目录过期，是机器后果）。

**验收**
| 项 | 结果 |
|---|---|
| `_check_qty_dialog_style.py` | **22/22** |
| `_reverse_verify_qty_dialog.py` | **9/9 注入报红** + 5 个被碰过的文件逐字节还原 |
| `QtyStepperTest`（JVM） | **8/8** |
| `_check_all.py` | **81/82**（唯一那条红是 `_check_backend_fresh`：本机 dev 后端比工作区旧 —— **别的会话** 08:38/08:49 改的后端文件，不是本轮改动；不重启的理由同上一条） |
| 真机（5556 货主，模拟器装机） | 选品页「＋」→ 新弹窗；下单页行编辑弹窗**同一形态**（见下） |

**真机上量出来的数（机器证据，不是肉眼）**：1080×2400 @420dpi（2.625px/dp）
- 单位标签「袋」文字右边界 871px + 10dp 内边距 = **897px**，与同一列的「¥34.8 / ¥104.4」右边界 **897px 完全相同**（贴最右）
- 数量框 `bounds=[439,1074][757,1221]` → 高 **147px = 56dp**（＝步进器整组高度，三格等高）；
  `−` / `+` 图标中心 y=1148 = 数量框中心 y=1147.5（对齐）
- **数字居中**：数量格里"墨"的像素重心 x=**597.7**，格子中心 x=**598.0**（偏 **0.3px**）
- **禁用态看得见**：数量=1 时 `−` 的墨色 `#8A8F9B`（灰）→ 数量=3 时 `#3E82FB`（蓝），`+` 恒为 `#3D82FB`
- 两个弹窗的步进器几何**逐像素相同**（`−` 339–397 / 数量框 439–757 / `+` 800–858）
- 端到端也没坏：数量改成 3 → 小计 ¥104.4（34.8×3）→ 确定 → 加入清单 → 下单页那一行是「数量 3 袋 · ¥104.4」
- 截图在 `_archive/qty-dialog-01-5556.png`（数量=1，`−` 灰）/ `-02-5556-qty3.png`（数量=3）/ `-03-5556-lineedit.png`（下单页行编辑弹窗）

⚠️ **本机 dev 后端当时跑的是旧代码**（`_check_backend_fresh` 那条红）：这一轮是**纯界面**改动，
数量/小计都是客户端算的，所以真机那几条结论不受影响；但"下单/加入清单"打的是本机后端（8000），
那一条只验到**界面与合计**，没有验后端落库（本轮没动后端一个字节）。

### [2026-09-23 09:0x → 09:2x] 会话：**修「更新提示看着像让我重装当前版本」**（版本号显示口径）+ 产品版本 0.2.3 → **0.2.4**、发 APK【已完成】（DSH `session-83da1ad7-e539-4d60-9412-b46a9a9dc48e`）

**用户原话**：「安装包的版本推送是不是有问题啊？他一直显示啊？**重安装当前版本**，但是你已经推送回新版本了，可**版本号的问题**嘛，**0.2.3 版本号并没有发生改变**啊，因为上个版本号也是这样子的」

**根因（代码级，不是猜的）**：`ProfileScreen.kt` 的确认弹窗
- 标题原来是 `"发现新版本 v" + vm.latest?.version` → 只有服务端的 **产品版本名**（`0.2.3`）
- 正文是 `"当前版本：v" + vm.currentVersion` → 是 **`产品版本 · 构建号`**（`0.2.3 · 2026092204`）

于是两行读起来是「发现新版本 v0.2.3 / 当前版本：v0.2.3 · 2026092204」——**产品版本一样**（同一天打几个包 `VERSION` 本来就不变），用户只能读成"让我重装当前版本"。
判新旧靠的**本来就是** `versionCode`（`ProfileViewModel` 的 `isNewer` 与安卓安装器都只认它），所以修法是**把它显示出来**。

**改了什么**
1. `ui/profile/UpdateProgress.kt`：新增纯函数 `versionLabel(version, code)` = `产品版本 · 构建号`（空版本兜「未知版本」；`code<=0/null` **不拼 0**）。口径只有这一处。
2. `ui/profile/ProfileScreen.kt`：确认弹窗标题改成 `UpdateProgress.versionLabel(vm.latest?.version, vm.latest?.versionCode)` → 「发现新版本 v0.2.4 · 2026092302」/「当前版本：v0.2.4 · 2026092301」。
3. **`VERSION` 0.2.3 → 0.2.4**（用户要的"版本号要变"）：这次发的是 24 个提交的一大捆改动，产品版本本来就该抬一格。
4. 单测 `UpdateProgressTest` +2 例（含**同一天两个包只差构建号**那种）；`_tools/deploy/_check_update_flow.py` +5 项（弹窗标题必须带构建号、两边口径一致、纯函数只有一处、不拼 0、单测钉着）。

**验收**
| 项 | 结果 |
|---|---|
| `_check_update_flow.py` | **43/43** |
| `UpdateProgressTest` | **8/8** |
| `_check_all.py` | 80/81（唯一那条红的是 `_check_backend_fresh`：本机 dev 后端比工作区旧 —— 08:38/08:49 那批**别的会话**改的后端文件，**不是本轮改动**；不重启是因为对方可能正改到一半，重启有把本机后端带崩的风险） |
| 真机（5556，模拟器） | 我的 → 关于与更新 → 检查更新：弹窗两行都带构建号（见下） |
| 重新发版 | **0.2.4 / versionCode 2026092302**（`--installed-code 2026092301` 多一道闸）；短链 `http://8.145.40.22/apk` 已指向新包；桌面只留 `SOrders派单送货-0.2.4.2026092302.apk`，与服务器包 sha256 一致（`e7823ffe…a08b406`） |

⚠️ **GitHub 又推不动**（Clash 代理 `127.0.0.1:7899` 仍未开、直连 github 也不通）：提交 `6166008` 已在本地，正在后台每 2 分钟重试推一次。**这一轮不需要为它发后端**：`VERSION` 在 backend 里没有任何消费点（`product_version` 在 `backend/` 里搜不到引用），产品版本只影响 APK 的 `versionName` 与界面显示。

### [2026-09-23 08:5x → 09:1x] 会话：**提交 + 部署生产（24 个提交）+ 发 APK 0.2.3（2026092301，含短链与桌面）**【已完成】（DSH `session-83da1ad7-e539-4d60-9412-b46a9a9dc48e`）

**用户原话**：「ok 将现有的提交一下 git，然后呢，打包出个 apk。推送到服务器端。以及放一个放在我的桌面，老旧的直接删掉」

**做了什么**
1. **提交**：我这一轮的代码与文档上一轮已提交（`c87e07c`）；查工作区时**只剩下两个文件是另一个会话正在改的**（`backend/app/core/schema_bootstrap.py`、`backend/app/models/order.py`，mtime 08:49，我看的时候 08:52）→ **没有替它提交**（都是核心区文件，且对方可能正改到一半）。
2. **部署生产**（`p:new`）：闸门 → 备份 → 推送 → 服务器 ff-merge → 重启 → 验收。
3. **发 APK**：0.2.3 / versionCode **2026092301**（`publish_apk.py --next-code` 算出来的；线上原为 2026092204）。
4. **桌面**：新包放桌面、旧包（`SOrders派单送货-1.0.0.20260917.apk`，48.1MB）删掉。
...
|---|---|
| `_check_all.py` | **77/77** |
| `pytest -q`（backend） | **804 passed**（95.6s） |
| 备份（服务器 `/root`） | `backup-sorders-deploy-20260923-0855.sql.gz`（533KB；自检：43 个 `CREATE TABLE` + `Dump completed`）+ `SOrders-backend-20260923-0855.tgz`（158MB） |
| 推送 | `9819752..4c706f3  p -> new`（**24 个提交**） |
| 服务器 | HEAD = `4c706f3` = `origin/new`；`systemctl restart` 后 **active / NRestarts=0**；health **200** |
| 库 | 表 43 → **44**（新表来自预订单分类那条线）；users 60 / orders 2402 / ledgers 4648 / products 37 —— **一行没变** |
| 生产端到端（只读） | 真账号 `15070334563` 登录 **200**（token 157 字符）→ `/users/me` 200（id=182 派单员）/ `/orders?limit=3` 200 / `/notifications?days=7` 200 |
| APK 闸门 | 签名指纹与线上一致（存量用户可直接升级）、**非 debuggable**、包内后端地址 = `https://8.145.40.22`、versionCode **2026092301 > 2026092204** |
| APK 上线 | `sorders-0.2.3-2026092301.apk`（**44,490,405 B**）→ `version.json` 回读一致；短链 `http://8.145.40.22/apk` = **200 / application/vnd.android.package-archive / 44490405 bytes**；中文 note 以 UTF-8 回读正常 |
| 桌面 | `SOrders派单送货-0.2.3.2026092301.apk`（42.4MB）；旧包已删（桌面现在只有这一个 apk） |

⚠️ **两件如实说明**

1. **GitHub 推送先失败过一次（网络）**：本机 Clash 代理（`127.0.0.1:7899`）没开、直连 `github.com:443` 也不通 → 第一次推送被 reset。
   **部署没有等它**：改用 `git bundle`（2.5MB，只含 `9819752..p`）传到服务器 `/root`，再 `git fetch <bundle> 'refs/heads/p:refs/remotes/bundle/p'` + `merge --ff-only refs/remotes/bundle/p`，把**同一批提交**送上去（服务器先到 `4c706f3`）；
   随后网络恢复，**第一次重试就推成功**（`9819752..4c706f3`），最后在服务器上 `git fetch origin` 把 `origin/new` 也对齐到同一个提交 —— **GitHub / 本地 / 服务器三处一致**（这也顺手消掉了"服务器从 bundle 前进、而 GitHub 落后"那个会让下一个人误判的窗口）。
2. 这次部署**连带发了另外两个会话的提交**（24 个提交里我只占 2 个）：`p:new` + `--ff-only` 天然做不到"只发一个提交"。**回滚点 = `9819752`**，备份见上表。
