...
| **`b0a364a`（当时的 tip）** | 绿 | 绿 | **整轮绿** |
| `a8cdc5d`（本报告重写版）/ `7030d25`（报告收尾） | 红 / 红 | 绿 / 绿 | ⚠️ 两次都**只有「安卓端到端」作业红**，而且都是**「连结果文件都没有 ＝ 模拟器压根没起来」**那一档 ⇒ **这次没有跑**（不是流程失败）。两个提交都只动了 `.md` |

⭐ 那个「安卓端到端」作业当天跑了 **13 次：绿 6 / `rc=1` 红 5 / 无结果文件 2**（无结果文件＝模拟器没起来，即**这次没有跑**）。
...
【R3-07】5ba7b1f  生成物新鲜度：四份产物各自声明真源指纹
        ...
        684a937  收口文档
【本次】本报告（重写版 v2）—— 提交号见 git log -1 -- docs/RECTIFICATION_REPORT_R3.md
```
