...
> | 项 | 值 |
> |---|---|
> | 版本 | **v2.2**（2026-09-26）；v2.1 之后**A 段（生产发布）已执行完成** —— 本版记的是它的结果 |
> | 交付日期 | 2026-09-26 |
...
> | 本轮提交数 | **62**（`git log --oneline c544db5..HEAD` 现算 —— ⚠️ 这个数每提交一次就 +1，v2.2 收尾时是 62，**含本节所在的这个提交**）|
> | 台账 | **53 ✅ / 7 ❌**（`docs/R3_PROGRESS.md`）—— A 段关掉 7 条，剩下的 7 条＝**B（2 条）+ C（5 条）**，两段都未放行 |
...
| **Production Runtime** | ✅ **部分（7/10）** | **A 段已走完**：生产跑 `b3dad61`、服务 0.2.4、库结构版本 8、写路径留下可追的痕迹（勾选表见 §2.6）；⛔ **B（socket / 多实例）与 C（五个演练）未放行** —— 所以只能说「部分」 | —— |
...
| 8 | Socket verified | ❌ **B 未放行** |
| 9 | Multi-instance verified | ❌ **B 未放行** |
| 10 | Failure drills verified | ❌ **C 未放行** |

⇒ **7 / 10**。⛔ 但用户 2026-09-26 明确要求：**不许把「Production Runtime Proven」简化成这一个分数**，要按四块分开写：

| 块 | 状态 | 说明 |
|---|---|---|
| **Production Runtime Foundation** | ✅ **7 / 10** | 上面那张勾选表：备份 / 迁移 / 启动 / 体检 / 只读烟测 / trace / 写烟测 ✅；socket / 多实例 / 演练 ❌ |
...
| **Failure Recovery** | ⛔ **未证明** | 五个演练 ＋ **备份的隔离恢复验证**（**C 段 —— 等 B 验收后再放行**）|
...
剩下的 7 条**没有一条卡在代码**，卡在「写许可」或「环境」，分属 B、C 两段：
...
| R3-03 socket 跨实例 | 1 | 本机没有 Redis；生产 keyspace 空、也没在用适配器 | **B**（可先装本机 Redis 只验这一格）—— ⭐ **2026-09-26 已放行** |
| R3-03 nginx 失败摘除 | 1 | 当前部署形态**根本没有 upstream**（单后端反代）| **B**（必须生产）—— ⭐ **2026-09-26 已放行**（但**排在 Socket 之后**）|
...
>
> **生产侧：A 段（发布）2026-09-26 已执行并验收**；**B（多实例）同日已放行、C 等 B 验收后再放行**。生产从 `648fbf8` 换到 `b3dad61`、服务版本 0.2.4、
> 库结构版本 8，写路径在生产上留下可追的痕迹（4 行审计带 command_id + request_id、3 条事件 sent）。
> ⛔ **但第三轮仍然没有完成**：剩下的 7 条 ❌ 是 **B（socket 跨实例 / nginx 失败摘除）** 与 **C（五个故障演练）**，
> 用户只放行了 A —— 所以「Production Runtime Proven」现在是 **7/10**，⛔ 不是 10/10。
