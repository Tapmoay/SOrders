...
> | 项 | 值 |
> |---|---|
> | 版本 | **v2.5**（2026-09-26）；v2.3 记 B 段（多实例），v2.4 记 **发布控制面与拓扑对齐 + 备份隔离恢复验证**，v2.5 记 **C 段：五个故障演练在生产上真跑过 + 演练抓到的两条真缺陷** |
> | 交付日期 | 2026-09-26 |
...
> | 本轮提交数 | **67**（`git log --oneline c544db5..HEAD` 现算 —— ⚠️ 每提交一次就 +1，v2.5 收尾时是 67，**含本节所在的这个提交**）|
> | 台账 | **59 ✅ / 1 ❌**（`docs/R3_PROGRESS.md`）—— A 段关掉 7 条、**B 段关掉 2 条**、**C 段关掉 4 条**；剩下的那 1 条＝ **R3-06 Drill C**，而它是**演练抓到的真缺陷**（投递失败被吞、事件被记成 `sent`），⛔ 不是「没跑」|
...
> | 生产 | **A 段已发布并验收**：生产跑 `b3dad61`（服务 0.2.4、库结构版本 8），「Production Runtime Proven」勾选 **9/10**；⭐ **B 段（多实例）与 C 段（五个故障演练）都已执行** —— 演练抓到的两条真缺陷见 §2.7 |
>
...
| R3-06 | Failure Drill（五个演练）| ✅ **5** / ❌ 1（**C 段已执行**：四条通过，一条抓到真缺陷）|
...
| **Production Runtime** | ✅ **9 / 10** | **A 段**（发布）+ **B 段**（两个实例 / socket 双向 / nginx upstream / 摘机测试）+ **C 段**（五个故障演练真跑过）都已完成；第 10 项「Failure drills verified」**算不过** —— 演练问的「故障能不能被发现」，它抓到的答案是「**发现不了**」（§2.7 发现 2）。勾选表与四块分法见 §2.6 | —— |

**一句话**：代码侧、CI 侧、本机运行层都已闭环；**生产运行层 A / B / C 三段都执行完了**（9/10）——
⛔ 但**剩下那 1 项不是「没做」，是「做了而且答不上来」**：故障演练抓到了真缺陷（§2.7）。
...
| 10 | Failure drills verified | ❌ **C 已执行，但算不过**：五条演练在生产上真跑过（4 通过 / 1 抓到真缺陷），⛔ 这一项判的是「故障能不能被发现」—— 抓到的是「发现不了」 |
...
### §2.7 R3-06 Failure Drill（✅ 5 / ❌ 1）——**五个演练在生产机上真跑过，并抓到两个真缺陷**
...
- 演练前先备份：`/opt/sorders-backup/pre_release/20260926T150728Z`；生产代码 `b3dad61`。
...
| B redis-down | `systemctl stop redis` | `/health` 如实变 `redis:error`；登录 / 读订单 / 读账本 / **写消息** 全 200；恢复后总线自己回来 | ✅ 5/5 |
| C event-delay | 停 Redis（让投递失败）| 业务写入成功、追平、不重复、净零；⛔ 但事件被记成 `sent attempts=0`，而日志里 **16 条** `Cannot publish to redis... giving up` | ❌ 5/6 |
...
**演练抓到两条**（⛔ 本轮只记录、**未改代码** —— 核心区冻结中）：

1. **自愈 bootstrap 的 DDL 没有服务端互斥**（Drill D 抓到）：`backend/app/core/schema_bootstrap.py:232` 只拿 `/tmp` 那把**本机**
   flock，服务端 `GET_LOCK` 是在**之后**的 `run_migrations` 里才拿的 ⇒ 跨主机时两个实例会真的同时跑自愈 DDL：
   三轮里每轮 1~2 处 **1213 死锁 / 1684 concurrent DDL statement**，并有一条 DDL 被**静默跳过**。
   ⚠️ 生产现状是**同机两个 unit**（共享 `/tmp` ⇒ 共享那把 flock）⇒ **没中招**；
   这是「跨主机多实例」的具体拦路石，与 `docs/MULTI_INSTANCE_READINESS.md` 的 `bootstrap-self-heal` 那一格对得上。
2. **发件箱把「投递失败」记成了「已发送」**（Drill C 抓到）：**python-socketio 的 Redis manager 把 publish 失败自己吞了**
   （记日志、不抛异常）⇒ `main.py::_outbox_deliver` 看到的是「成功」，重试/退避/`failed`+`last_error` **一条都不会触发**。
   这正是 outbox 模块开头点名要治的病（「那条推送就没了 —— 数据库里一切正常，所以没人会发现」）：
   边界治住了业务事务与事件，却在**最外层的 emit** 上漏了回来。后果：跨实例那条推送**永久丢掉且没有任何地方记着**
   （站内信的行还在、客户端重连会重新拉 ⇒ 伤的是**实时性**，不是数据）。
   修法方向与判据写在 `docs/R3_FAILURE_DRILL_EVIDENCE.md` §三·发现 2。

⚠️ 另有一条**未定**：Drill B 记录里 `R_NUMSUB=1`（两个实例里只有一个订阅着跨实例总线），
重启 + 逼 emit 之后仍是 1；更像 python-socketio 的监听器**惰性启动**，但本次**没有**量到
「客户端连到 A 时 A 会不会开始订阅」⇒ 如实记成未定，⛔ 不写成缺陷。
...
| ~~R3-06 演练五条~~ | ~~5~~ | ⭐ **已完成**（2026-09-26 C 段：五条都在生产上真跑过，六阶段 X0–X5）| **C ✅**（⚠️ 其中 event-delay 抓到真缺陷，见 §2.7 —— 演练做完了，但那一格的结论是「不通过」）|
...
| 生产真的跑过 + 故障演练 | ✅（2026-09-26 A 段：发布八步走完） | ✅ | **部分**：生产**跑过** ✅（9/10 勾选项，见 R3-05）；**故障演练** ⚠️ 五条**都在生产上真跑过**（4 通过 / 1 抓到真缺陷）—— ⛔ 但第 10 项仍是 **❌**，因为那一条判的是「故障**能被发现**」 |
