...
| 只读烟测 | ✅ | 生产实测：**❌ 0 条**（30 通过 / 2 既知告警）；request_id 直连与经 nginx 都原样回来；⚠️ 判据从「退出码 0」改成「❌ 0 条」，见下 |
...
剩下的 7 条**没有一条卡在代码**，卡在「写许可」或「环境」，分属 B、C 两段（台账同款表在 `docs/R3_PROGRESS.md` 的「写阶段出口」一节）：
...
| R3-06 演练五条 | 5 | 停服务 / 断 Redis / 塞磁盘（生产侧）| **C**（本机预演已 5/5，⛔ 形态不同不能顶替）—— **未放行** |
| R3-03 socket 跨实例 | 1 | 本机没有 Redis；生产 keyspace 空、也没在用适配器 | **B**（可先装本机 Redis 只验这一格）—— **未放行** |
| R3-03 nginx 失败摘除 | 1 | 当前部署形态**根本没有 upstream**（单后端反代）| **B**（必须生产）—— **未放行** |
...
>
> **生产侧：A 段（发布）2026-09-26 已执行并验收** —— 生产从 `648fbf8` 换到 `b3dad61`、服务版本 0.2.4、
