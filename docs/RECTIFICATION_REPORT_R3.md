...
> | 本轮提交数 | **57**（`git log --oneline c544db5..HEAD` 现算 —— ⚠️ 这个数每提交一次就 +1，v2.1 收尾时是 57，**含本节所在的这个提交**）|
...
## §10 本轮提交台账（**57 个**，`c544db5..HEAD`；⛔ 不写死 tip 提交号 —— 现算）
...
【本次】CI 记录补最后两行（取消一次 / 收尾提交红在模拟器）—— 提交号见 git log -1
```

完整 **57** 条用 `git log --oneline c544db5..HEAD` 现算（⚠️ 含本节的收尾提交本身）。
