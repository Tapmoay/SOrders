...
| 语音播报 | 司机端与派单员端：见 §5「新单提醒」——**不用系统 TTS**，用固定音频素材 + 重复播报（TTS 只做兜底） |

---

## 5. 新单提醒（系统通知 + 语音播报 + 后台常驻）

需求原话（司机端）：「派单员派的单给他，他那个要有对应的消息通信还有语音播报——说来，来单了来单了，
大概 3 秒钟，可以重复多次，就像货拉拉的样子」。
需求原话（派单员端，2026-09-21）：「派单员**收到订单的时候**、他**有订单需要派**的时候，
他会有个**语音播报**，就跟我们的司机是一样的」。
下面这三层缺一层，用户那头就只是「今天没响」。

⚠️ **两个角色各听一句**（司机听「有新派单」、派单员听「有新订单待派单」）：
判据是**角色 × 事件类型**两维，唯一实现在 `core/NewOrderAlert.kt` 的 `speaks()`
（只按角色判的话，派单员会对着司机那句「请及时查看」发呆）。
...
| **语音播报** | 循环播「古典号角 + 一整句话」（一段 ≈5 秒 × 用户设置的次数）：司机 `new_order.wav`、派单员 `pending_order.wav` | `core/NewOrderPlayer.kt` + `res/raw/{new_order,pending_order}.wav` |
...
| 维度 | 规则 |
|------|------|
| 谁播 | **司机听「有新派单」、派单员听「有新订单待派单」**（`speaks(role, kind)`，两维一起判）；货主一句都没有——「来单了」对他是错的信息 |
| 播什么 | 新派单（`order.assigned`）→ `new_order.wav` = **古典号角（大小交替，≈1 秒）+ 语音「来订单了，你有新的订单，请及时查看」**；待派单（`order.created`，后端那条「新订单待派单」站内信）→ `pending_order.wav` = 号角 + 「来订单了，有新订单待派单，请及时处理」；撤回/取消 → 只说一遍（复用司机那份音频） |
| 播几次 | 用户可选 1 / 2 / 3 次 / 一直响（司机文案「到我接单」、派单员「到我派完单」，默认 3 次）；「一直响」有 60 秒止损 |
| 语音怎么来的 | **微软神经语音（edge-tts）**，不是手机/系统 TTS——第一版用 Windows SAPI 合成，用户听完的评价是「很机器人」，神经语音才有真人感（离线时自动退回 SAPI） |
| 去重 | 同一次派单后端会从 `notification` 与 `realtime` 两条链路各推一次，60 秒内同一单只响一次（司机键 `assigned:{id}`、派单员键 `pending:{id}`，互不干扰） |
| 何时闭嘴 | 司机：接单 / 已送达 / 撤回 / 取消；派单员：**自己派完这一单** / 别人接了单 / 已送达 / 订单被撤销；两边都还有 **点开通知** / 用户关掉开关 |
...
- **重新生成音频**：`python _tools/media/_gen_new_order_clip.py [--kind driver|dispatcher]`
  （号角用 numpy 合成；语音走 edge-tts 神经语音，**离线时自动退回 Windows SAPI**）。
  换音色：`--voice zh-CN-XiaoxiaoNeural`；挑音色：`--list-voices` / `--candidates`（多音色各出一份试听）。
  改完素材必须同步改 `NewOrderAlert.CLIP_MS`（司机那类）/ `PENDING_CLIP_MS`（派单员那类）——
  脚本会直接打印该填的数字，红线还会拿 wav 头对账（素材清单是从脚本的 `KINDS` 里解析出来的）。
- **音色**：两次播报都是**晓晓（`zh-CN-XiaoxiaoNeural`，女声）**——用户 2026-09-17 听完云健（男声）
  那一版后指定的。⛔ 生成时**别传 `--voice`**（脚本默认就是晓晓）；想确认成品是哪个音色：
  `python _tools/media/_probe_clip_voice.py`（量语音段基频：女声 ≈200~280Hz、男声 ≈90~140Hz）。
...
| 2026-09-21 | §5 扩到**两个角色**：派单员也听语音（`order.created` → `pending_order.wav`「来订单了，有新订单待派单，请及时处理」，用户拍板"新录一句专用的"）；判据从"只有司机"改成**角色 × 事件类型**两维（`speaks`）；派单员后台常驻默认开。⚠️ 同日修：派单员那句第一版误用了脚本默认音色（云健＝男声），用户一听就发现了 → 默认值改成他选的**晓晓（女声）**，并新增 `_tools/media/_probe_clip_voice.py`（量基频）+ 红线对账 |
