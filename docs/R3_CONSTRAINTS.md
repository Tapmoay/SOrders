...
> **R3 收口提交**：`c67b2cf`（R3 的最后一个提交）
> ⚠️ **这个右端点是 R4-00（2026-09-27）加的，理由值得写下来**：上面第 5/7/8 组探针原来一律算
> `基线..HEAD` —— 那个窗口**永远不会关闭**，于是 R4 每加一个扩展、每动一行 `backend/app`，
> 都在消耗 **R3 的额度**；R4 干几件事之后，「R3 净增不许超上限」会突然报红，而原因跟 R3 无关。
> **一个只属于某一轮的判据，它的窗口必须跟着那一轮一起收口。**
> ⛔ 但两件事**故意跨轮继续生效**：① 每个提交都要有可回溯的里程碑编号（仓库级长期纪律）；
> ② 新增检查器必须写「为什么代码边界解决不了」（R4 指南 §30 有同样要求）。见 `_check_r3_constraints.py` 文件头。
...
| 日期 | 条目 | 从 | 到 | 为什么 | 什么时候收回去 |
| --- | --- | --- | --- | --- | --- |
| 2026-09-26 | `backend_app_delta` 的总闸 | 500 | 800 | 它原来把「结构契约与设施」也算成业务代码量。R3-01/R3-02 往 `core/` 加的是 `file_lock.py`（跨平台锁）、`role_capabilities.py`（角色门事实）、`capability_audit_coverage.py`（审计覆盖）—— **业务逻辑（`services/**` + `api/**`）净增其实是 0 行**。只拿总数当判据，会逼着人为了好看去砍注释。 | R3 结束时结算：业务逻辑那一栏仍 ≤60 行就两个数一起保留；超了说明这一轮跑偏了，先查业务逻辑 |

**同时新增了一个更紧的闸门**：`MAX_BUSINESS_DELTA = 60`（只算 `services/**` + `api/**` 的净增）——
这才是原则一真正要防的东西。

| 日期 | 条目 | 从 | 到 | 为什么 | 什么时候收回去 |
| --- | --- | --- | --- | --- | --- |
| 2026-09-26 | `backend_app_delta` 总闸 | 800 | 1200 | 它又把结构工作算进去了：R3-03/R3-04 新增的 `scheduler_lock.py`、`command_id.py`、迁移 008、指标与日志格式都是**运行时边界**，而**业务逻辑净增只有 12 行**（上限 60）。⛔ 但这次不是单纯抬数：**同时加了两个 shape 闸门** —— 见下。 | R3 结束时结算：业务逻辑 ≤60 行且新模块 ≤9 个 / 单个 ≤220 行，就保留；任何一项破了先查那一项 |

**2026-09-26 新增的两个 shape 闸门**（比总行数那把钝刀准得多）：

· `MAX_NEW_FILE_LINES = 220` —— 新增的 `.py` 里**任何单个**不许超过 220 行（防「一个巨大的新模块」）；
· `MAX_NEW_FILES = 9` —— 本轮允许新增的 `.py` 总数（防「一口气加一堆模块」）。

当前实测（`python _tools/qa/_check_r3_constraints.py`）：

```text
backend/app 净增 901 行（上限 1200）；新模块 6 个（上限 9，最大的 176 行 / 上限 220）；
业务逻辑（services+api）净增 12 行（上限 60）
```
