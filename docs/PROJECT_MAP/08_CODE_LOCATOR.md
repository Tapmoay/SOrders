...
| 账本管理 | `DispatcherLedgerScreen.kt` / `DispatcherLedgerViewModel.kt`。**2026-09-19 重做（用户点名）**：原话「批发商他只能看到合计的，但如果我想看**单个**的呢？或者我想看 **2 个**人的呢？这要有个**自由选择**，而且页面也非常的反人性」、「账本相近的明细，比如他这个账本对应什么订单，**订单是可以展开进行查看的**」。<br>· **账户搜索 + 多选**（`AccountPicker`）：一个都不选 = **全部**（默认行为不变，不逼用户先选一次才能看账）；选中态可点掉、有一键「清空选择」；下面那两句「一个都不选 = 全部（共 N 个）」是**必需**的 —— 多选控件最常见的困惑就是"我什么都没点，现在显示的是全部还是什么都没有"。<br>· **选中账户的合计**（`SelectedSummary`）：原来只有每张卡各自的钱，要看"这几家一共多少钱"得自己心算 —— 用户说的"反人性"就包括这条。⚠️ 笔数取**服务端**的 `count`（全量），不是本地明细行数（那是分页的）。<br>· ⛔ **货主账与批发商账是同一支代码**（`when { 2, 3 -> }`）：原来是两段几乎一样的复制，改一处漏一处。<br>· **明细里那一单就地展开**：`ui/common/OrderPeek.kt`（派单员账本与货主账本**共用一份**）。原来点一行是**跳到订单详情页** —— 跳过去再回来，筛选/展开的账户/滚动位置全没了，连着核几笔要来回跳十几趟；跳转那条路仍然留着（展开块右上角「打开订单」，看照片/导航/司机备注还得进详情页）。<br>· ⚠️ 换 tab 时**清空账户选择**（`selectTab`）：key 是 `u|id` / `t|名字`，在货主账与批发商账里指的是不同的人，带过去会出现"选了 2 个、列表里一个都没高亮"的鬼状态 |
