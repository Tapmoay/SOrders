...
| 端 | 功能 | 文件 |
|---|---|---|
| 货主 | 下单 | `ui/shipper/OrderCreateScreen.kt` / `OrderCreateViewModel.kt`（**2026-09-19：地址库抽屉改成两栏** —— 用户：「我们也改成类似商品管理的形式，因为我们有 **3 个分组**嘛，3 个分组就放在左侧，然后右边就是对应的地点」。原来三枚横向胶囊（`SegmentedStatusTabs`）只能显示"现在在哪一段"，两栏能**同时看到三段各有多少条**（线路 8 条 / 我的地点 9 条 / 共享地点 3 条）。左栏同样是 `MasterRail`（无副标题 → 行高自动 52dp，选中竖条用主题主色）；搜索框仍在左栏**上面**横跨整页，**三段共用**，`if (tab == 2) onSearchPlaces(...)` 那条"共享地点顺带打后端"的线保持不动（红线 `_check_ai_guardrails.py` §31 钉着）） |
