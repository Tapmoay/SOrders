...
| **共享地点库**（**全库共用**的一张表） | `backend/app/api/v1/places.py`、`backend/app/services/place_service.py`（**距离判据与"名字地址不能都空"的唯一一处实现**）、`backend/app/schemas/place.py` | `app/api/v1/shipper.py::share_location`（设为共享）、`android/.../ui/shipper/OrderCreateScreen.kt::AddressPickerSheet`（行尾 `⋮`）、回归测试 `backend/tests/test_place_manage.py`（10 例） | 用户 2026-09-19：「这个地址是可以编辑的。如果是来到了共享库的话，共享地址的编辑**只有派单员**可以编辑，其他人都编辑不了。派单员可以**改名称**，也可以把一些地点给**设置为共享地址**，也可以**撤销**某些共享地址，把它**降为普通的地址**，或者直接**删掉**。如果是降为普通的地址的话，则这个地址会保存在**派单员**的地址库当中，其他的不会显示」。<br>· **四个动作**：`PATCH /places/{id}`（改名称/地址，**坐标刻意不给改**——那是"某个位置在哪儿"的事实，指错了司机照着走就是错的）、`DELETE /places/{id}`（**物理删除**）、`POST /places/{id}/demote`（撤销 → 存进**操作人自己**的「我的地点」+ 从共享库删掉）、`POST /shipper/locations/{id}/share`（设为共享：输入是**地点编号**，坐标从那一条上取）。<br>· ⛔ **权限一律 `require_roles(DISPATCHER)`**：`POST /places`（记录一个点）三种角色都能走——司机在现场、货主在门口，谁有坐标谁补；而改/撤/删动的是**别人也在用**的那条记录，所以要一个人负责。<br>· ⚠️ **删除是物理删除**（与主数据那套 `SoftDeleteMixin` 不同）：一条记录就是"名字+一对坐标"，删了重新标一次就有；软删要在**每一处**查询补 `is_deleted` 过滤，漏一处就是"库里明明有一模一样的点、补录却说没找到"（`find_place_near` 也吃这张表）。代价是**没有回收站**，所以删除前把整行写进 `operation_logs`。`place_user_usage` 必须一起删（外键 + 它没有主语）。<br>· **AI 也有一套**（用户点名：「再给派单端的 AI 去增加这些功能」）：写能力域「共享地点」4 个动作 `place.update/delete/demote/publish`（不进 `SHIPPER_ACTIONS` ⇒ 货主默认没有）。⛔ 模型**全程碰不到经纬度**：`POST /places` 仍然在 `_write_coverage` 的"永久排除"里，而"设为共享"的输入是**地点编号**。<br>· 撤回：`place.update` 有（写回旧值）；撤销/删除/发布没有，理由写在 `AiRevert.none(...)`（删除那条补一句"内容在操作日志里"）。 |
