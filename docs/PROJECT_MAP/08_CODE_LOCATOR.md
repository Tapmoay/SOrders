...
| **地点分组管理** | `PlaceCategoriesScreen.kt` / `PlaceCategoriesViewModel.kt`。**按人分区**（货主和派单员各管自己那一份，互相看不到）。入口只有一条：下单页地址库左栏**底部那格「管理分组」**（用户圈着截图说「就在这个红框的位置，它跟左边那一个分组类别是一个**对齐**的状态」）。<br>· 建（顶部输入框 + 新建）、改名（弹窗，**级联改掉挂着的地点**）、删（**还有地点挂着时后端拒绝**）、**排序**（上下箭头，一步一格）。<br>· ⚠️ 排序提交的是**整份顺序**（后端 `/reorder` 要 `ids` 覆盖全部分组，只传一部分会 400）；箭头只是把"整份"藏在一次点击里。<br>· ⚠️ 位次显示用**列表下标**，不是 `sortOrder + 1`（后端新建给 `max+1`，第一个是 1）—— 真机抓到显示成 2。<br>· 分组是**可选**的：不建分组，地址库照样用默认那三段 |
