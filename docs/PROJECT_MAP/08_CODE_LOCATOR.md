...
| **报表中心** | `app/api/v1/reports.py`（638 行）、`app/services/stats_service.py`（425 行） | `app/services/stats_export.py` | **毛利必须带覆盖率说明**（只算 `cost_price_snapshot > 0` 的行），否则毛利虚高。<br>⚠️ **毛利的收入侧与成本侧必须是同一批行**：`cost_covered_amount − cost_total`（营业纵览 / 商品页 / 导出表头 / 导出行 / App 全部读它）。曾经三处三个数：App 商品页 11,071 / 正确 10,789 / 导出逐行合计 72,177.75（2026-09-19 审计）；逐行没有成本快照时印「—」并说明"不进毛利"。<br>⚠️ 绩效/导出的「计费方式」走 `driver_pay.snapshot_mode`（挂了规则看规则），不许读 `users.billing_mode`；**也不许从窗口内的订单快照里随便取一个**（同一窗口可能两种都有，取任意一个是掷骰子） |
...
| 商品管理 | `ProductsScreen.kt` / `ProductsViewModel.kt`。**版式 = 左边分类、右边商品**（用户 2026-09-19：「这个商品管理也做成选择商品的那种界面，左边是分类右边是商品」）—— 分类导航条与"商品属于哪一类"的判据都复用选品页那一份（`ui/common/ProductPicker.kt` 的 `CategoryRail` / `categoryTabs` / `categoryOf`，都是 `internal`），**不许各写一份**（各写一份会出现"同一件商品在选品页属于日化、在商品管理页属于未分类"）。<br>**商品卡上只有两个数字**（售价橙 + 库存蓝青；**成本与分类都不在卡上**，**一个数字一行**，库存还按状态变色）—— 见 §2.3 上一行与 `06_DESIGN_SYSTEM.md §4`，改之前先读那几条理由；动作（各批发商价格/上架下架/编辑/删除）在卡片右上角 `⋮` 菜单里，**它下面还有一个「改价」快捷入口**（**紫色**，只改默认售价，用户 2026-09-19 要求：「下面比较空嘛，在下面再加一个改价…方便、快捷」；⛔ 颜色**不许**用钱的橙，会和旁边的售价撞色），**不许**把动作再摊成三个按钮（用户：「太占位置了」）。<br>⚠️ 快捷改价的弹窗预填价要用 `util/Money.kt::trimMoneyZeros`（去尾零、不丢精度），**不是** `formatMoney`（两位小数，会把 12.3456 显示成 12.35 = 骗人）。<br>⛔ 商品编辑页**没有"批发价档位"**了（2026-09-19 删）：那个概念看着像批发价、下单时谁都不照它走，批发商的价格只有「各批发商价格 / 定价」两个入口设 |
