...
| # | 方法与路径 | handler | 位置 | 授权 |
|---|---|---|---|---|
| 1 | `DELETE /api/v1/orders/{order_id}` | `delete_cancelled_order` | `backend/app/api/v1/orders_lifecycle.py:39` | 角色:dispatcher\|shipper + 体内权限:ORDER_DELETE_CANCELLED + 体内含角色判断（需读源码） |
| 2 | `POST /api/v1/orders` | `create_order` | `backend/app/api/v1/orders_lifecycle.py:103` | 权限:ORDER_CREATE + 体内仅允许:派单员\|货主 |
| 3 | `PATCH /api/v1/orders/{order_id}` | `update_order` | `backend/app/api/v1/orders_lifecycle.py:275` | 权限:ORDER_EDIT |
| 4 | `PATCH /api/v1/orders/{order_id}/exception` | `patch_order_exception` | `backend/app/api/v1/orders_lifecycle.py:344` | 权限:ORDER_EDIT |
| 5 | `POST /api/v1/orders/{order_id}/restore` | `restore_order` | `backend/app/api/v1/orders_lifecycle.py:385` | 角色:dispatcher |
