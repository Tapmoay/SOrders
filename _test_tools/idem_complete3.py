...
async def main():
    ...
    async with aiohttp.ClientSession(connector=conn) as sess:
        ...
        body = {"lines": [{"product_id": 1, "product_name_snapshot": "双写测试3", "quantity": 5, "unit_price": "23.00"}], "address_detail": "双写3", "contact_boss_phone": "13800000001"}
        ...
        form.add_field("files", io.BytesIO(png), filename="p1.png", content_type="image/png")
        async with sess.post(BASE + f"/api/v1/orders/{oid}/delivery-photos", headers=H_DRV, data=form, timeout=15) as r:
            print("upload:", r.status, (await r.text())[:100])
