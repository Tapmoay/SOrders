# -*- coding: utf-8 -*-
"""SOrders 混合负载压测 v2 (BASE参数化)"""
import sys, json, time, asyncio, random, argparse
...
async def shipper_worker(sess, stats, phone, stop_evt):
    ...
    while not stop_evt.is_set():
        action = random.choice(["list_orders", "create_order", "get_detail", "list_products", "ledger"])
        if action == "list_orders":
            ...
            s, d, _ = await raw(sess, "GET", "/api/v1/ledger/entries?from=2026-09-01&to=2026-09-30", h=h)
            stats.record("shipper/ledger", s, d)
        else:
...
async def dispatcher_worker(sess, stats, stop_evt):
    ...
    h = {"Authorization": f"Bearer {tok}"}
    while not stop_evt.is_set():
        action = random.choice(["pool", "orders", "assign", "reports", "ledger", "users", "products"])
        if action == "pool":
            ...
            s, d, _ = await raw(sess, "GET", "/api/v1/products", h=h)
            stats.record("dispatcher/products", s, d)
...
async def main():
    ...
    async with aiohttp.ClientSession(connector=conn) as sess:
        ...
        print(f"[{args.prefix}] 配置: 派单员1 + 货主{n_shipper} + 司机{n_driver} = {N_USERS} @ {BASE}", flush=True)
        tasks = [asyncio.create_task(shipper_worker(sess, stats, f"1391000{(i % 60) + 1:04d}", stop_evt)) for i in range(n_shipper)]
        tasks += [asyncio.create_task(driver_worker(sess, stats, f"1382000{(i % 50) + 1:04d}", stop_evt)) for i in range(n_driver)]
        ...
        print(f"\n===== [{args.prefix}] {N_USERS}并发 {DURATION}s @ {BASE} =====", flush=True)
