...
print("[%s] 预登录 %d 用户..." % (PREFIX, N_USERS), flush=True)
accs = []
for i in range(n_ship):
    accs.append(("shipper", SHIPPERS[i % len(SHIPPERS)]))
for i in range(n_drv):
    accs.append(("driver", DRIVERS[i % len(DRIVERS)]))
for role, phone in accs:
    tok = login(phone)
    accs_data = getattr(sys.modules["__main__"], "accs_data", None)
    if not accs_data:
        accs_data = []
        sys.modules["__main__"].accs_data = accs_data
    accs_data.append((role, phone, tok))
tok_d = login("13800000001")
print("预登录完成: %d/%d (dispatcher %s)" % (sum(1 for _,_,t in accs_data if t), N_USERS, "ok" if tok_d else "fail"), flush=True)

def shipper_worker(role, phone, tok):
    if not tok: return
...
def driver_worker(role, phone, tok):
    if not tok: return
    while not stopevt.is_set():
        t0 = time.time()
        st = get("/api/v1/orders", tok)
...
def dispatcher_worker():
    while not stopevt.is_set():
        ...
        r = random.random()
        if r < 0.3:
            st = get("/api/v1/orders?status=PENDING_DISPATCH", tok_d)
            record("disp/pool", st, time.time() - t0)
        elif r < 0.5:
            st = get("/api/v1/reports/turnover?mode=day&date=2026-09-04", tok_d)
            record("disp/reports", st, time.time() - t0)
        elif r < 0.7:
            st = get("/api/v1/orders", tok_d)
            record("disp/orders", st, time.time() - t0)
        else:
            st = get("/api/v1/users?limit=100", tok_d)
            record("disp/users", st, time.time() - t0)
        time.sleep(random.uniform(0.8, 2.0))

print("启动 %d 并发线程..." % N_USERS, flush=True)
threads = []
for role, phone, tok in accs_data:
    if role == "shipper":
        threads.append(threading.Thread(target=shipper_worker, args=(role, phone, tok)))
    else:
        threads.append(threading.Thread(target=driver_worker, args=(role, phone, tok)))
