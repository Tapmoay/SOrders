...
/**
 * 分类排序的两段纯逻辑：`moveCategoryTo`（数字排序与拖动共用）与 `dragSteps`（位移→格数）。
 *
 * ## 为什么这两条必须单测
...
class ProductCategoriesOrderTest {
    ...
    @Test
    fun `填几就排到第几（1-based）`() {
        assertEquals(listOf(1L, 3L, 4L, 2L, 5L), ids(moveCategoryTo(five, id = 2, position = 4)))
        assertEquals(listOf(1L, 4L, 2L, 3L, 5L), ids(moveCategoryTo(five, id = 4, position = 2)))
        assertEquals(listOf(2L, 1L, 3L, 4L, 5L), ids(moveCategoryTo(five, id = 2, position = 1)))
        assertEquals(listOf(1L, 3L, 4L, 5L, 2L), ids(moveCategoryTo(five, id = 2, position = 5)))
    }

    @Test
    fun `填 0 或超过总数会被夹到两端（不报错、也不丢行）`() {
        assertEquals(listOf(3L, 1L, 2L, 4L, 5L), ids(moveCategoryTo(five, id = 3, position = 0)))
        assertEquals(listOf(3L, 1L, 2L, 4L, 5L), ids(moveCategoryTo(five, id = 3, position = -7)))
        assertEquals(listOf(1L, 2L, 4L, 5L, 3L), ids(moveCategoryTo(five, id = 3, position = 99)))
        // 夹完之后仍然是这 5 个，一个不多一个不少
        assertEquals(five.map { it.id }.toSet(), moveCategoryTo(five, id = 3, position = 99).map { it.id }.toSet())
    }

    @Test
    fun `挪到原位时原样返回同一个列表实例`() {
        // ViewModel 靠 `===` 判断"没变就别写状态"，这条契约不能破
        assertSame(five, moveCategoryTo(five, id = 3, position = 3))
        // 已经夹到两端之后也一样（第 1 位填 0、最后一位填 99）
        assertSame(five, moveCategoryTo(five, id = 1, position = 0))
        assertSame(five, moveCategoryTo(five, id = 5, position = 99))
    }

    @Test
    fun `找不到的 id 与不足两行时不改任何东西`() {
        assertSame(five, moveCategoryTo(five, id = 999, position = 2))
        val one = listOf(cat(1))
        assertSame(one, moveCategoryTo(one, id = 1, position = 3))
        val empty = emptyList<ProductCategoryDto>()
        assertSame(empty, moveCategoryTo(empty, id = 1, position = 1))
    }
    ...
    fun `拖动是一格一格换位，所以中途松手也不会丢行`() {
        ...
        list = moveCategoryTo(list, id = 1, position = 2)   // [2,1,3,4,5]
        list = moveCategoryTo(list, id = 1, position = 3)   // [2,3,1,4,5]
        list = moveCategoryTo(list, id = 1, position = 4)   // [2,3,4,1,5]
