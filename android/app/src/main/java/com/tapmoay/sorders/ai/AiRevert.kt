...
object AiRevert {
     ...
     *
     * 为什么值得判：金额和非金额的写法**不一样** —— 金额走显示口径（到分为止 + 去尾零，
     * `5.00 → 5`），非金额只去尾零、**按自己的精度**写（`2.5000 → 2.5`、`1.005` 不能被凑成 `1.01`）。
     * 同一张卡上两行写法不同会让用户以为是两件事。
     */
    ...
    private fun buildUndoNone(none: (List<String>, String) -> Unit) {
        ...
        // ---- 退货：撤回它的唯一办法是"把货再卖回去"，而那已经不是同一件事了 ----
        none(
            listOf(AiWrites.ORDERS_RETURN),
            "退货撤不回来：它已经写了一条账本红冲行、把库存补回去了，可能还退了一笔现金 ——" +
                "要纠正请在「订单管理」里重新开一张单（货又买回去了就是新的一笔生意），" +
                "或者到账本里手工补一笔反向流水，别指望一键撤回",
        )
        // ---- 退货申请：**申请**与**执行**要分开说（2026-09-21）----
        //
        // 这一组的误操作答案与上面那条**完全不同**，混成一句就把用户最需要知道的
        // 那件事盖掉了：申请阶段什么都没动（撤不撤都没有账/库存后果），
        // 而办理之后与 `orders.return` 一样撤不回来。四条各说各的。
        none(
            listOf(AiWrites.RETURN_REQUEST_APPLY),
            "这张申请发出去之后，这条消息上没有「撤回」按钮（撤回要按申请单撤，" +
                "而它在写之前还没有编号）。要撤就说一句「撤回退货申请」—— 那是一个独立动作" +
                "（`return_request.withdraw`）。⚠️ 顺带说清：申请阶段账本、库存、订单状态一样都没动，" +
                "所以不撤也没有任何后果；真正撤不回来的是派单员办理之后那一步",
        )
        none(
            listOf(AiWrites.RETURN_REQUEST_WITHDRAW),
            "撤回本身撤不回来：撤回之后那张申请就作废了（派单员也办不了它了）。" +
                "要恢复就重新申请一次 —— 数量是锁死的，所以想改数量也只能这么改。" +
                "记录留着（撤回不是删除），派单员看得到你提过又撤了",
        )
        none(
            listOf(AiWrites.RETURN_REQUEST_REJECT),
            "驳回发出去就发出去了：理由已经推给货主，后端没有「取消驳回」这个动作。" +
                "想让货主重新申请，请他再提一张就行（驳回本身不动账、不动库存、不改订单）",
        )
        none(
            listOf(AiWrites.RETURN_REQUEST_FULFILL),
            "办理就是真退货：账本已经按那几行红冲、货已经补回库存、收过钱的还退了一笔现金 ——" +
                "撤不回来。要纠正请重新开一张单（货又买回去了就是新的一笔生意），" +
                "或者到账本里手工补一笔反向流水",
        )
        // ---- 补导航：只补不改，补完也没有"取消导航"这个动作（2026-09-20）----
        none(
            listOf(AiWrites.ORDERS_FILL_NAV),
            "补导航这个动作只补不改（后端对已有坐标的单直接拒绝 —— 把对的坐标改成错的" +
                "比没有坐标更糟），所以它没有「撤回」这一说：要改坐标得派单员在「改单」里" +
                "带上经纬度重发一次（那是另一个动作，见 AiWrites.ORDERS_UPDATE）。" +
                "这也是为什么卡片上会把坐标数字写在脸上：先核对，再确定",
        )
        // ---- 供应商付款：钱真的出去了，所以理由要**说清怎么收回来**（2026-09-22）----
        //
        // 它写的是**新的一行资金流水**（`cash_flows` 的 `PAYMENT_SUPPLIER`），编号在写之前不存在
        // —— 与「核销」同一个形状。但后果重得多：那一行流水会立刻算进「还欠他多少」与账本「收支」。
        // ⛔ 所以**不用**那句通用的"建错了就改/删"兜底：用户看到"可以删掉"会以为删了就等于没付过，
        //    而这里要他说清的是**撤销付款**（那是一笔一笔撤的动作，撤掉之后欠款立刻变回来，
        //    流水行还在库里、随时能恢复）。
        none(
            listOf(AiWrites.SUPPLIER_PAYMENT_PAY),
            "付款这一步没有「撤回」按钮（新写的那一行流水在写之前还没有编号）；" +
                "要收回来说一句「撤销给 XXX 的那笔 XXX 元付款」——那是「撤销付款」那个动作：" +
                "撤销之后「还欠他多少」立刻变回去、账本「收支」里也不再算这一笔，" +
                "而且那一行流水还在库里（后台是伪装删除），随时能恢复",
        )
        // ---- 货主自己那一本账：核销记下去之后，撤回要按订单去撤（2026-09-20）----
        //
        // 为什么不给它一键撤回：核销写的是**新的一条**，它的编号在写之前不存在
        // （`AiRevert.plan` 是"在 commit 之前拍现场"，那时这一条还没有）。
        // 但用户要的"撤掉"是能做的 —— 那是一个**独立动作**（`my_ledger.revoke`），
        // 所以这里如实说清"该说哪句话"，而不是留一个点了没反应的按钮。
        none(
            listOf(AiWrites.MY_LEDGER_SETTLE),
            "核销记下去之后，这条消息上没有「撤回」按钮（新记的那一条在写之前还没有编号）；" +
                "要撤就说一句「撤掉订单 XXX 的那笔核销」——那是一笔一笔撤的动作，" +
                "撤掉之后记录还在（后台是伪删除），随时能再恢复",
        )
        ...
        // ⚠️ 2026-09-23 补的三份名册（开销 / 运费 / 预订单分类）**各写一句**：
        //    单测 `撤不回来的理由必须一条一句，不许一句万能话糊住一批动作` 给"共用"设了上限
        //    （同一条理由最多覆盖 2 个动作），而这三份顺序**后果各不相同**：
        //    开销那一份决定"这笔钱算哪一类"、运费那一份决定"这类货走哪条价目"、
        //    预订单那一份只决定"我这几张常用单怎么分组"。逐条写清才对得上用户看到的东西。
        none(
            listOf(AiWrites.EXPENSE_CATEGORY_REORDER),
            "开销分类的顺序是「整份名册」一起提交的（不是改某一条记录的一个字段），" +
                "撤回入口只认「一条记录写回旧值」，对不上这个形状。" +
                "要改回去，把想要的完整顺序再说一遍（例如「加油、修车、过路费」），我照着重排",
        )
        none(
            listOf(AiWrites.FREIGHT_CATEGORY_REORDER),
            "运费分类的顺序是「整份名册」一起提交的（顺序决定运费模板页与计费规则页里那一栏的先后），" +
                "撤回入口只认「一条记录写回旧值」，对不上这个形状。" +
                "要改回去，把想要的完整顺序再说一遍（例如「日杂、生鲜、建材」），我照着重排",
        )
        none(
            listOf(AiWrites.ORDER_TEMPLATE_CATEGORY_REORDER),
            "预订单分类的顺序是「整份名册」一起提交的（它只决定预订单页左栏怎么分组、谁排前面），" +
                "撤回入口只认「一条记录写回旧值」，对不上这个形状。" +
                "要改回去，把想要的完整顺序再说一遍（例如「常送、临时」），我照着重排",
        )
    ...
    private val CREATE_LIKE: Set<String> = setOf(
        ...
        // 给供应商付款（2026-09-22）：它写的是**新的一行资金流水**（`cash_flows`），
        // 编号在写之前不存在 —— 与上面几个同一个形状。
        // ⚠️ 但它的后果**比"建错了就改/删"重得多**（钱真的出去了），所以下面 UNDO_NONE 里
        //    单独给它写了一条**说得清怎么收回来**的理由，而不是用那句通用兜底。
        AiWrites.SUPPLIER_PAYMENT_PAY,
    )
