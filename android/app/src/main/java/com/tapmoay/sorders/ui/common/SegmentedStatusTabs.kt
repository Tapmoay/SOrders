...
/**
 * 订单状态导航色：全部 / 派单中 / 已接单 / 已送达 / 已撤销 / 已退货 / 已派单。
 * ⛔ 这一行里**不许**让"斜杠"紧跟"加粗的开头"（Kotlin 的块注释**会嵌套**：那种写法等于又开了一层
 *    注释，而它永远等不到配对的收尾，实测编译报 `Unclosed comment`）—— 2026-09-24 第 19 轮在这里
 *    踩过一次：改这一行时写成了"已退货" + 斜杠 + 加粗的"已派单"，编译器直接拦下。
 *
 ...
 * ⚠️ **已派单（`DISPATCHED`）只能加在最后**（2026-09-24 第 19 轮补的那一档）：
 *    业务顺序上它排在「派单中」与「已接单」之间，但色表是按**位置**取的 ——
 *    插到第 3 格会把「已接单/已送达/已撤销/已退货」四档的颜色一起顶掉，
 *    那是用户在真机上会立刻看到的界面变化（而这次要修的只是"这一档没有入口"）。
 *    `RETURN_TAB_COLORS`（两页退货申请）取的是 `[1]`/`[0]`，加在末尾也不受影响。
