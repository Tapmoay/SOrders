...
class AiAgentLoop(
    ...
    private fun systemPrompt(enabledToolNames: List<String>, systemExtra: String? = null): String {
        ...
        return buildString {
            ...
            appendLine("6. 金额写「元」并**去掉末尾多余的 0**：56.70 写 56.7、87.00 写 87、0.50 写 0.5；")
            appendLine("   但**有零以外的位一位都不能少**（56.77 就写 56.77，不许写成 56.7 或 57）。")
            appendLine("   日期一律 YYYY-MM-DD；先给结论再列明细，多条数据用列表。")
            ...
            // ⚡ 2026-09-21 用户要求「给 AI 开一个捷径：既能批量操作，又能单个调整」：
            //    一次说要改多条时**必须一次提交**（`items`）——一条一条地调既费 token，
            //    又会在界面上堆出一串确认卡（用户得逐张点，中途漏一张他就会以为全做完了）。
            //    落地在 `ai/AiWriteBatch.kt`：一张卡列全部行、确认一次、逐条如实汇报。
            appendLine("   - ⚡ **一次要改/要建好几条时，用「批量」一次提交，不要一条一条地重复调用**：")
            appendLine("     同一个操作只调一次，`params` 里传 `items` 数组（每项一条，键名与单条完全一样）。")
            appendLine("     系统会把它们合成**一张卡**逐条列给用户，他确认一次就全部执行，并逐条汇报成败。")
            appendLine("     范围**由用户定**（他说「全部」就是全部，没有条数上限）：所以先按上面那条把名册查全，")
            appendLine("     再逐条写进 `items`；一次调用只放**同一个操作**（不同操作各发一次）。")
            appendLine("     ⛔ 只有一条时就用普通参数，不要套 `items`。")
            appendLine()
