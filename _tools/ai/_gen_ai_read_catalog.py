...
CN_DESC = {
    ...
    # 开销 / 运费分类名册（2026-09-23 新开）：这两张原来在 `_read_coverage.EXCLUDED` 里
    # （理由："模型要按分类看开销时，那几张读表每行都带分类名，不用先查名册"）。
    # 这一轮把三张名册的建/改名/排序/删都开给了模型，**读名册就从"用不上"变成了前置条件**：
    # 重排要整份顺序（少一个后端整份拒绝）、改名/删除前要报出"这一类下挂着几笔开销/几条价目"
    # —— 那两个数字就在名册的 `expense_count` / `template_count` / `rule_count` 上。
    "expense_categories.list_categories": (
        "开销分类名册（「开销管理」左栏那一列的名字与顺序，带每类下有**几笔开销**；"
        "⚠️ 改名会级联改掉挂在这一类下的开销记录）"
    ),
    "freight_categories.list_categories": (
        "运费分类名册（「哪几类货」那张配置表：名字、顺序，带每类下挂着**几条价目**与**几份计费规则**；"
        "⚠️ 还有价目/规则挂着时不许删）"
    ),
