...
MAP: dict[str, tuple[list[str] | str, list[str] | str, str]] = {
    "派单作业": (["订单/派单", "统计口径"], ["订单"], "看池子 + 批量派单/撤回派单"),
    "待派单池": (["订单/派单"], ["订单"], "同上，是派单作业的一个视图"),
    "全部订单": (["订单/派单", "订单商品行"], ["订单"], "改单/拆单/删单/商品行增删改"),
    # 模板的增删改挂在 G_ORDER 里（`AiWriteBasicData.kt` 的 FREIGHT_TEMPLATE_* 三条），
    # 不是"只能读"——一开始我按"模板只是参考数据"想当然写成了缺口，工具当场把它顶出来。
    "订单模板": (["订单/运费模板"], ["订单"], "运费模板增删改（动作在订单域里）"),
    ...
    "地址与联系人": (["地址与联系人", "共享地点库"], ["地址与联系人"], "线路/地点/联系人增删改 + 回收站恢复"),
    ...
    "账本管理": (["账本", "现金流水", "费用", "客户"], ["账目"], "流水/收款/支出/删流水"),
