...
EXCLUDED: dict[tuple[str, str], str] = {
    ...
    # AI 调用计数上报（2026-09-25 新增端点，报告 §15 ② 的 AI_calls）。
    # ⛔ 理由不是"懒得做"，而是**它的语义决定了模型不能碰**：它收的是 App 报上来的
    #    「我刚跑了几次对话」，而这个数直接喂给 `sorders_ai_calls_today` ——
    #    让模型能调它，等于让模型**给自己的指标刷数**。指标一旦能被被观察者改，它就不再是证据。
    #    （与之相对：`AI_write_confirmed` 是后端从 `operation_logs` 数出来的，模型同样改不了。）
    # ⚠️ 等 Android 侧真的开始上报之后，可以考虑挪进 `AI_DIRECT_USE`（那一桶的语义是
    #    「AI 功能自己在用、但入口不在模型手上」）—— 现在还没接，挪过去会当场变化石。
    ("POST", "ai/telemetry"): "AI 调用计数上报：让模型能调它等于让模型给自己的指标刷数 ——"
                               "指标一旦能被被观察者改就不再是证据",
