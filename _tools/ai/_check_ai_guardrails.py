...
def main() -> int:
    ...
    # 三条拦截
    # ⚠️ 2026-09-24 第 21 轮：文案从「个**司机**挂着这份规则」改成「个**账号**挂着」——
    #    闸门不再按 `role == DRIVER` 数（改一次角色就能绕过它，见 R20-C6-2），
    #    锚点跟着改、**不放宽**（仍然要求"点明还有几个人挂着"）。
    c.present("还挂着司机时不许删（删了那批人会静默退回老算法）", rule_api, r"个账号挂着这份规则")
