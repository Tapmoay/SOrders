...
def main() -> int:
    ...
    c.present("提示词明确「成本价既看不到也改不了」", loop, r"成本价你既看不到也改不了")
    ...
    # ---- 2g-3 成本是红线：不许进参数，也不许由模型设 ----
    # 成本价一旦进模型上下文，它就出现在聊天记录里、可能被截图外发。
    # 所以"能不能改成本"这个问题的答案必须是"不能"——不是"能但要注意"。
    c.absent("商品动作的参数表里没有成本价字段", wmaster, r'"(cost|cost_price)"')
    c.present("并且写明了成本不在 AI 手上的理由", wmaster, r"成本价：未设置")
    c.present("落库时不传 cost_price（用 DTO 默认值）", wsvc, r"cost_price 刻意不传")
