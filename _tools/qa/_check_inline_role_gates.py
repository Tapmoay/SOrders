...
HISTORY: list[tuple[str, int, str]] = [
    ...
    ("2026-09-25", 38, "§9 第②步第四域：products 的读门接到 `require_permission` 上（11→10 个文件）"),
    ("2026-09-25", 36, "§9 第②步第五域：orders_lifecycle 的 2 处**无条件**门收到签名上（双角色 + 纯派单员）；"
                              "同文件另 2 处是**条件**门（货主专属权限点 / 角色链的 else），留在体内 ——"
                              "所以它**不进 CONVERTED 表**（那张表要求体内为 0）"),
]
...
CONVERTED: dict[str, str] = {
    ...
    "app/api/v1/products.py":
        "2026-09-25 §9 第②步第四域：读门接到 require_permission（Permission.ORDER_CREATE）上，体内 0 处",
}
