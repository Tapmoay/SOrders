...
#    多一个（新增软删模型）不用动它。
#
# ⛔ 2026-09-24 第 24 轮（第 22 轮 F6 报的"当年那个空转形状又长回来了"）：
#    软删模型已经涨到 **15** 个、端点到 **19** 处，而下限还停在 9 / 8 / 6 ——
#    再删掉 6 个软删模型（含 CashFlow / SupplierPayable 这种动钱的）这条红线**照样绿**。
#    下限重新对齐到"当前实际数量"：**少一个就要有人解释**。
#    ⚠️ 新加一个软删模型不需要改这里（判据是 `>=`）；数字变小时才需要解释。
MIN_MODELS = 15
MIN_ENDPOINTS = 19
#: 「会改一行 + 取过软删模型」的端点数量下限（实测 19，与 [MIN_ENDPOINTS] 同一个数）。
#: ⚠️ 第 22 轮 F6 报告说它"定义了从未被引用"——**那是错的**（它在 `main()` 里判空转，见下面
#: `if len(pairs) < MIN_PAIRS`）。本轮按实测把它从 6 提到 19，顺手纠正那条结论。
MIN_PAIRS = 19
