...
创建与 PATCH 两条写路径 + 搜索**两条** OR 分支。
Android：`OrderDto` 两个字段、下单请求两个字段、下单页两个输入框、
自动填的两条来源（账号资料 → 下单人；线路 → 收货人）、卡片与详情都显示（共用 `contactWho`）。
...
def main() -> int:
    ...
    # ---- ② 后端：两条写路径 ----
    c.present("创建路径把两个名称写进 Order(...)", api,
              rf'{RECEIVER}=body\.{RECEIVER}\.strip\(\)[\s\S]{{0,200}}?{ORDERER}=body\.{ORDERER}\.strip\(\)')
    ...
    c.present("两条自动填的入口在 init 里被调到", vm, r"init \{[\s\S]{0,900}?prefillOrderer\(")
