...
### ⭐ 这三层是「自己写的核对」，还有一层是**外部证明**（2026-09-26 补）

L1/L2/L3 判的都是**每份脚本自己写的那几句**。而自己写的核对可能**盯错一头** —— 实测：
`path, original = _c, _t` 那句让「还原后核对」比的是**同一份错字节**，恒等、静默通过。
所以真正的外部证明在 `_reverse_verify_all.py`：**每跑完一份就逐字节比对「现场 vs 开跑前的快照」**，
不一致就点名（哪一份、哪些文件）＋当场按快照写回＋计为不达标。那一条**不依赖任何脚本的自述**。
