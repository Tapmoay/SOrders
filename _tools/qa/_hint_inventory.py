...
OVERRIDE: dict[tuple[str, str], tuple[str, str]] = {
    ...
    # ↓ 单位换算页（2026-09-24）那一句：它是**说明这一页设完之后会怎样**（静态、不随数据变），
    #   被判成 DATA/DATA 是同一类误伤 —— 句子里「1 车 = 8 方」「10 车 ≈ 80 方」里的
    #   **车 / 方 / 单位** 全命中单位词表。方向 DATA → EXPLAIN，理由：它是解释句，不是状态数字。
    ("UnitConversionsScreen.kt", "设一条「1 车 = 8 方」这样的换算之后"): (
        "EXPLAIN", "解释句：说明设完换算之后数量会长什么样、以及**钱不受影响**；"
                   "判成 DATA 是「车 / 方 / 单位」命中单位词表"),
    ...
    # ↓ 2026-09-22：预订单那一套（列表页 / 新建编辑页 / 分类管理页）的 5 句 `Hint(...)`。
    #   它们都是**解释句**（"这是什么""为什么这页没有价""顺序从哪来"），判成 DATA 是
    #   同一个已知偏差：**「预设单」「预订单」里的「单」命中单位词表**（与上面
    #   `ProductFormScreen.kt` 那两条「下**单**」的诊断一字不差）。
    #   ⚠️ 这一批尤其不能藏错方向 —— 用户 2026-09-22 原话就是要它们归提示总开关管：
    #   「这个解释没必要…或者说你可以把这个解释**绑到那个提示当中**，按提示的形式来」。
    ("OrderTemplatesScreen.kt", "预设单就是你常用那一单的模板"): (
        "EXPLAIN", "这一页的「它是什么」说明（用户点名要它归提示总开关）；"
                   "判成 DATA 是「预设**单**」里的「单」命中单位词表"),
    ("OrderTemplatesScreen.kt", "这里是你删掉的预设单"): (
        "EXPLAIN", "回收站那一句说明（在哪、怎么放回来）；同上被「预设单」误伤"),
    ("OrderTemplateFormScreen.kt", "预设单不含单价"): (
        "EXPLAIN", "**业务事实的说明**（为什么这一页没有价、价在哪一刻算）；"
                   "它必须能被总开关关掉，否则新建页永远挂着一长条解释"),
    ("OrderTemplateFormScreen.kt", "预设运费只是参考值"): (
        "EXPLAIN", "同上（运费为什么只是参考值：下单接口根本不收运费）"),
