    ...
    ok(
        ...
        all(
            any(k in s for k in (
                ...
                #    而账本页自己的用词就是"日期档位"（页面文案与代码注释都这么叫）。
                #    这条检查的本意是"**文案必须指向页面上真的有的东西**"，
                #    不是"必须凑齐某几个词" —— 逼它改口只会让同一个控件出现两种叫法。
                "日期档位",
                # ⚠️ 2026-09-19 新增「可能不全」：有一类页面**没有任何筛选入口** ——
                #    「各批发商价格」（`PriceMatrixScreen.kt`）的名单是**服务端截断**的，
                #    客户端搜索搜不到根本没取回来的那些行，所以连"用搜索框找"都是做不到的话。
                #    这种页面唯一诚实的说法是"这份名单可能不全，别据此下结论"。
                #    这条检查的本意是"**不许承诺做不到的事**"，不是"必须凑齐某几个词"，
                #    所以收下这句话 —— 它同样只能出现在 `TruncationNote` 的文案里。
                "可能不全",
            ))
