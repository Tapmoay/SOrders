    ...
    for _n, jn, j in e2e_jobs:
        ...
        if esc:
            ...
            want(0 <= i_grant < i_py,
                 ...
                 "（第 59 轮实测）")
        # ---- 21. 端到端要的**业务数据**也得播（只有账号不够）----
        # ⛔ 2026-09-25 第 60 轮实测：三条主链里「登录 ✅ 导航 ✅」都过了，「下单」卡在
        #    `❌ 商品选择页里点不到「?」那一行的 ＋。当前屏幕：选择商品 ｜ 搜索商品名 ｜
        #     暂无可用商品 ｜ 请联系派单员先在「商品管理」中添加商品后再下单 ｜ …`
        #    —— 因为 `seed_dev_users` 只造三个账号：选品页是空的、地址库也是空的。
        #    判据钉的是**调用形状**（不是「正文里提到过某个脚本名」）：本项目栽过
        #    「判据被一段文字提及满足」，所以这里连 `python -m` 一起钉。
        want("python -m scripts.seed_ci_e2e" in body,
             jn + " 播了端到端要的业务数据（`python -m scripts.seed_ci_e2e`）",
             "⛔ " + jn + " 只造了账号、没造商品/地址 —— 「下单」那一段会在选品页上卡住"
             "（`暂无可用商品`），而前两段仍然全绿，看起来像「只有下单坏了」（第 60 轮实测）")
