"""红线：订单详情里的**司机电话**——只有派单端给「拨号」按钮，且拨出去的号一定是能拨的。
...
| 写坏的方式 | 表现 |
|---|---|
| 把按钮的 `role == Role.DISPATCHER` 那道门去掉 | 货主/司机自己也多出一颗按钮 —— 用户明说过"其他人是没有的"，而且没人会为"多了一颗按钮"报错 |
...
| 顺手把整行藏给非派单端 | 用户只说了**按钮**只给派单端；司机是谁这一行本身三个角色都看（派单员靠它认人） |
| 改了核心文件（`order_response.py`）却不更新定位表 | 下一个人按地图去读，读到的是"没有这段处理"的那一版 |

## 判据
Android：`ui/order/OrderDetailScreen.kt` 里 `DriverRow` 的定义与调用、按钮只在派单端、
`ACTION_DIAL`、不可拨就不画、图标与语义色俱在、按钮与信息**同排**。
后端：`services/order_response.py` 下发的 `driver_phone` 过 `strip_del_suffix`（软删后缀去尾）。
配套：后端用例、设计规范 §5 那条偏好、定位表那一行。
...
def main() -> int:
    ...
    # 调用点窗口：从锚点向两边展开（不要把整页都卷进来，"派单端"那道门必须在**这一行**上）
    ...
    # ⚠️ 拆成三条、别揉成一条：揉起来之后"只给派单端"和"号码不可拨就不给"会互相掩护 ——
    #    去掉 `&& dialable` 时前一条照样绿（`dialable` 这个 val 还在上面算着，只是没人用）。
    c.present("**拨号按钮只在派单端**（开关里有 `role == Role.DISPATCHER`）",
              block, r"onDial = if \(role == Role\.DISPATCHER\b")
    c.present("**号码不是能拨的形状就不给按钮**（`dialable` 真的接进了开关）",
              block, r"onDial = if \(role == Role\.DISPATCHER && dialable\) \{")
    ...
    c.present("设计规范里写了这条偏好（「拨号」按钮 + `role == Role.DISPATCHER` 挨着写）",
              design, r"拨号[\s\S]{0,400}?Role\.DISPATCHER|Role\.DISPATCHER[\s\S]{0,400}?拨号")
    ...
    if total < 18:
        print(f"❌ 只跑了 {total} 条判据（< 18）—— 判据在空转，停。")
        return 1
    if c.fails:
        ...
        for f in c.fails:
            print("   - " + f)
        return 1
    print(f"✅ 全部 {c.passes} 项通过：司机电话只有派单端能拨，且拨出去的号是能拨的。")
    return 0
