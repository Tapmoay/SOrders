...
## 顺序（必须在同一个维护窗口里做完）
```
① mysqldump 备份（脚本会打印命令）
② 部署本轮代码（Python 写 UTC + 会话时区 UTC）
③ 立刻跑本脚本 --apply        ← 不跑的话老数据仍是 +08:00
④ 验证：SELECT 一行老数据，它的 created_at 应当比"服务器当地墙上时间"早 8 小时
⑤ 重启 uvicorn（会话时区钩子要新连接才生效）
```
