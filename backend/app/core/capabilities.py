...
## 现在：一张声明 + 一份被钉住的镜像（⛔ 不是「rbac 由本表派生」）
每个权限点在这里**声明一次**。`rbac.py` 的 `ROLE_PERMISSIONS` 与 `SCOPES` **仍然是手写的那一份** ——
它们是核心区，且别的判据锚着它的段落结构，改成派生会把那些锚点连根拔掉（风险远大于收益）。
所以现状是**两份互为镜像**，由判据 `_tools/qa/_check_capability_registry.py` 做**双向一致**核对：
改一处忘一处就报红，效果与「只有一份」相同。判据核对两件事：
...
## 与 AI / UI / 审计的关系（⚠️ 做到哪一步就说哪一步）
- **AI**：✅ 已同源 —— 读能力的角色裁剪走 `rbac.ROLE_PERMISSIONS`，而它与本表由上面的双向判据钉住。
- **UI**：⛔ **本轮没做** —— 安卓侧某个按钮显不显示，仍是各页自己的角色判断，**不是**读本表。
  指南 §十一 要的 API/AI/UI/Audit 四端同源，本轮只做到 API 与 AI 两端。
- **审计**：⛔ **本轮没做** —— `models/enums.OperationAction` 的动作码与 `kind=write` 的能力还没有对照判据
  （现有的只是「每个动作码有中文名」那条）。这两条写在这里是**欠账**，不是已完成的事实。
"""
