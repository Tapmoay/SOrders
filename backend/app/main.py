...
async def _outbox_loop() -> None:
    ...
    ⚠️ 这段 docstring 原来写的是「本轮**只立边界与 worker**，还没有生产者往里写……每 2 秒扫一次空表」——
    那句话早就过期了（2026-09-25 盘点：39 个入队点、18 种事件，判据 `_check_outbox.py` 逐个核过）。
    留在这里的教训是：**解释性文字也会腐烂，而它腐烂时没有任何地方会报错** ——
    所以凡是能被判据核对的事实，都写成表让判据去核对（聚合根映射、处理器登记表都是这么来的）。
    """
