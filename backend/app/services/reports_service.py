"""
报表的**聚合逻辑**（2026-09-24 整改阶段 4「reports.py 下沉」）。

报告 §6 点名的两处 API 层过厚之一：「reports 的聚合逻辑也直接写在 API 层」。
本轮把计算整体搬到这里（纯搬迁：口径 / 四舍五入 / 窗口换算 / 覆盖率说明一字未改），
`api/v1/reports.py` 只留路由、鉴权与出参模型。

为什么下沉而不是再拆几个 api 文件：报表是**读侧聚合**，消费方不止 HTTP（导出 / 报表中心 /
AI 读能力都要同一批数字）；放 service 层才能复用，放 api 层只能互相 import —— 那正是
「同一个数两处算」的开端。

⚠️ `api/v1/reports.py` 仍 **re-export** 这些名字：既有用例与判据是按
`app.api.v1.reports` 这个路径引用它们的，搬迁不改引用点。
"""
